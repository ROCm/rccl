...
// operates on buffer from index 512 to 767. Each gathers data from all the gpus
// into its registers and does reduction operation on them. Using the above
// example, gpu 2 gather data from index 512 to 767 from all the gpus into its
// registers and does floating point addition on them. The final result is
// stored into local destination buffer. Then, each gpu gathers rest of the data
// from other gpus.
//
