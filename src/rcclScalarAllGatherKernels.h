...
__global__ void RcclKernelScalarAllGather(RingNode_t* pcurr_track,
                                               int rank, int count) {
    int tx = threadIdx.x;
    int bx = blockIdx.x;
    ...
    const DataType_t* curr_src_buff = reinterpret_cast<const DataType_t*>(pcurr_track->src_buffer);
    ...
    while (pnext_track->rank != rank) {
        ...
        //! buffer
        if (tid < count) {
            curr_dst_buff[tid + curr_rank * count] =
                next_src_buff[tid];
        }
