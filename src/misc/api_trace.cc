...
#define RCCL_ASSERT_OFFSET(TABLE, MEMBER, IDX)                                                    \
    ...
    RCCL_ASSERT_OFFSET(rcclApiFuncTable, mscclLoadAlgo_fn,                  31);
    RCCL_ASSERT_OFFSET(rcclApiFuncTable, mscclRunAlgo_fn,                   32);
    RCCL_ASSERT_OFFSET(rcclApiFuncTable, mscclUnloadAlgo_fn,                33);
    RCCL_ASSERT_OFFSET(rcclApiFuncTable, ncclCommRegister_fn,               34);
    RCCL_ASSERT_OFFSET(rcclApiFuncTable, ncclCommDeregister_fn,             35);


#undef RCCL_ASSERT_OFFSET

    static_assert(
        sizeof(rcclApiFuncTable) == compute_table_size(36),
...
ncclResult_t ncclCommInitRankConfig_impl(ncclComm_t* comm, int nranks, ncclUniqueId commId, int myrank, ncclConfig_t *config) {
...
ncclResult_t mscclRunAlgo_impl(
    ...
    size_t count, ncclDataType_t dataType, int root, int peer, ncclRedOp_t op,
    mscclAlgoHandle_t mscclAlgoHandle, ncclComm_t comm, hipStream_t stream){
         return ::rccl::RcclGetFunctionTable()->mscclRunAlgo_fn(sendBuff, sendCounts, sDisPls,
                recvBuff, recvCounts, rDisPls, count, dataType, root, peer, op,
