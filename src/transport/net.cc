...
static_assert(sizeof(ncclNetHandle_t) <= CONNECT_SIZE,
              "NET Connect info is too large");
...
#define NCCL_NET_MAP_GET_POINTER(mapStruct, cpuOrGpu, offsetName)              \
  (NCCL_NET_MAP_OFFSET_NULL(mapStruct, offsetName)                             \
       ? NULL                                                                  \
       : (mapStruct)                                                           \
                 ->mems[NCCL_NET_MAP_OFFSET_BANK(mapStruct, offsetName)]       \
                 .cpuOrGpu##Ptr +                                              \
...
#define NCCL_NET_MAP_ADD_POINTER(mapStruct, shared, dev, memSize, offsetName)  \
  do {                                                                         \
    int bank = NCCL_NET_MAP_MASK_USED + (dev) * NCCL_NET_MAP_MASK_DEVMEM +     \
               (shared) * NCCL_NET_MAP_MASK_SHARED;                            \
    if ((shared) == 0) {                                                       \
      if (dev) {                                                               \
        (mapStruct)->offsets.offsetName =                                      \
            bank + (mapStruct)->mems[NCCL_NET_MAP_DEVMEM].size;                \
        (mapStruct)->mems[NCCL_NET_MAP_DEVMEM].size += memSize;                \
      } else {                                                                 \
        (mapStruct)->offsets.offsetName =                                      \
            bank + (mapStruct)->mems[NCCL_NET_MAP_HOSTMEM].size;               \
...
struct connectMap {
  ...
  int shared;
  int cudaDev;
  // First 3 bits of offsets determine the mem bank. 001 is host mem, 011 is dev
  // mem, 101 is shared host mem and 111 is shared dev mem.
...
struct sendNetResources {
  ...
  size_t maxP2pBytes;
  volatile uint32_t
      *curr_hdp_reg; // Curr GPU in ring (for rdma transport use only)
};
...
static_assert(
    sizeof(ncclNetHandle_t) + sizeof(int) <= CONNECT_SIZE,
...
static ncclResult_t
sendSetup(struct ncclComm *comm, struct ncclTopoGraph *graph,
  ...
  send->conn.shared = req.shared =
      (graph || connIndex == 0 || mscclIsCaller()) ? 0
  ...
  if (connIndex == NCCL_CONN_IDX_P2P_NET)
    NCCLCHECK(ncclTopoGetIntraNetDev(comm->topo, myInfo->rank, graph, channelId,
                                     1, &netId, &req.netDev));
  if (req.netDev < 0)
    NCCLCHECK(ncclTopoGetNetDev(comm, myInfo->rank, graph, channelId,
                                peerInfo->rank, &netId, &req.netDev,
                                &proxyRank));
  NCCLCHECK(ncclTopoCheckGdr(comm->topo, myInfo->rank, netId, 1, &req.useGdr));
  send->conn.flags |= req.useGdr ? NCCL_DIRECT_NIC : 0;
  if (!req.useGdr && connIndex == 0)
    comm->useGdr = 0;
  if (proxyRank != myInfo->rank && connIndex == 0)
    comm->useNetPXN = true;
  if (req.useGdr &&
    ...
    CUDACHECK(hipDeviceGetAttribute((int *)&req.curr_hdp_reg,
  ...
  NCCLCHECK(
      ncclProxyConnect(comm, TRANSPORT_NET, 1, proxyRank, &send->proxyConn));
  ...
  if (proxyRank == myInfo->rank) {
    INFO(NCCL_INIT | NCCL_NET,
         "Channel %02d/%d : %d[%lx] -> %d[%lx] [send] via NET/%s/%d%s%s comm "
         "%p nRanks %02d",
         channelId, connIndex, myInfo->rank, myInfo->busId, peerInfo->rank,
         peerInfo->busId, comm->ncclNet->name, req.netDev,
         req.useGdr ? "/GDRDMA" : "", req.shared ? "/Shared" : "", comm,
         comm->nRanks);
  } else {
    INFO(NCCL_INIT | NCCL_NET,
         "Channel %02d/%d : %d[%lx] -> %d[%lx] [send] via NET/%s/%d(%d)%s%s "
         "comm %p nRanks %02d",
         channelId, connIndex, myInfo->rank, myInfo->busId, peerInfo->rank,
         peerInfo->busId, comm->ncclNet->name, req.netDev, proxyRank,
         req.useGdr ? "/GDRDMA" : "", req.shared ? "/Shared" : "", comm,
         comm->nRanks);
  }
  *((int *)connectInfo) = comm->topParentRanks[proxyRank];
  memcpy((uint8_t *)connectInfo + sizeof(ncclNetHandle_t), &req.useGdr,
         sizeof(int));
  return ncclSuccess;
}
...
static ncclResult_t
recvSetup(struct ncclComm *comm, struct ncclTopoGraph *graph,
          struct ncclPeerInfo *myInfo, struct ncclPeerInfo *peerInfo,
          struct ncclConnect *connectInfo, struct ncclConnector *recv,
          int channelId, int connIndex) {
  struct setupReq req = {0};

  recv->conn.shared = req.shared =
      (graph || connIndex == 0 || mscclIsCaller()) ? 0
      : ncclParamNetSharedBuffers() != -2          ? ncclParamNetSharedBuffers()
                                                   : 1;
  req.channelId = channelId;
  ...
  if (req.netDev < 0)
    NCCLCHECK(ncclTopoGetNetDev(comm, myInfo->rank, graph, channelId,
                                myInfo->rank, &netId, &req.netDev, &proxyRank));
  ...
  if (req.useGdr) {
    NCCLCHECK(
        ncclTopoNeedFlush(comm, req.netDev, myInfo->rank, &req.needFlush));
    CUDACHECK(hipDeviceGetAttribute((int *)&req.curr_hdp_reg,
                                    hipDeviceAttributeHdpMemFlushCntl,
                                    myInfo->cudaDev));
  ...
  NCCLCHECK(
      ncclProxyConnect(comm, TRANSPORT_NET, 0, myInfo->rank, &recv->proxyConn));

  req.tpLocalRank = comm->topParentLocalRanks[comm->localRank];
  req.tpRank = comm->topParentRanks[myInfo->rank];
  req.tpRemoteRank = comm->topParentRanks[peerInfo->rank];
  NCCLCHECK(ncclProxyCallBlocking(comm, &recv->proxyConn, ncclProxyMsgSetup,
                                  &req, sizeof(req), connectInfo,
                                  sizeof(ncclNetHandle_t)));
  memcpy((uint8_t *)connectInfo + sizeof(ncclNetHandle_t), &req.useGdr,
         sizeof(int));
  INFO(NCCL_INIT | NCCL_NET,
       "Channel %02d/%d : %d[%lx] -> %d[%lx] [receive] via NET/%s/%d%s%s comm "
       "%p nRanks %02d",
       channelId, connIndex, peerInfo->rank, peerInfo->busId, myInfo->rank,
       myInfo->busId, comm->ncclNet->name, req.netDev,
       req.useGdr ? "/GDRDMA" : "", req.shared ? "/Shared" : "", comm,
       comm->nRanks);
  return ncclSuccess;
}

static ncclResult_t netMapShm(struct ncclComm *comm,
                              struct connectMapMem *mem) {
  NCCLCHECK(ncclShmImportShareableBuffer(
      comm, &mem->createDesc, (void **)&mem->cpuPtr, (void **)&mem->gpuPtr,
      &mem->attachDesc));
  return ncclSuccess;
}

static ncclResult_t netCreateShm(struct ncclProxyState *proxyState,
                                 struct connectMapMem *mem) {
  NCCLCHECK(ncclShmAllocateShareableBuffer(
      proxyState->tpRank, mem->size, false, &mem->createDesc,
...
static ncclResult_t netDumpMap(struct connectMap *map) {
  ...
  printf("SendMem -> Used %d Bank %d Offset %x, cpu %p gpu %p\n",
         ...
         NCCL_NET_MAP_OFFSET_BANK(map, sendMem),
         map->offsets.sendMem & NCCL_NET_MAP_MASK_OFFSET,
         NCCL_NET_MAP_GET_POINTER(map, cpu, sendMem),
         NCCL_NET_MAP_GET_POINTER(map, gpu, sendMem));
  ...
  for (int p = 0; p < NCCL_NUM_PROTOCOLS; p++) {
    printf("Proto %d -> Used %d Bank %d Offset %x, cpu %p, gpu %p\n", p,
           ...
           NCCL_NET_MAP_OFFSET_BANK(map, buffs[p]),
           map->offsets.buffs[p] & NCCL_NET_MAP_MASK_OFFSET,
           NCCL_NET_MAP_GET_POINTER(map, cpu, buffs[p]),
...
static ncclResult_t sendConnect(struct ncclComm *comm,
                                struct ncclConnect *connectInfo, int nranks,
                                int rank, struct ncclConnector *send) {
  struct connectMap *map = (connectMap *)send->transportResources;
  void *opId;
  int recvUseGdr;

  memcpy(&recvUseGdr, (uint8_t *)connectInfo + sizeof(ncclNetHandle_t),
         sizeof(int));
  if (!recvUseGdr)
    send->conn.flags &= ~NCCL_DIRECT_NIC;
  ...
  if (!map) {
    ...
    NCCLCHECK(ncclProxyCallAsync(comm, &send->proxyConn, ncclProxyMsgConnect,
                                 &args, sizeof(netSendConnectArgs),
  ...
  if (map->sameProcess && !ncclCuMemEnable()) {
    if (map->cudaDev != comm->cudaDev) {
      ...
      if (err == cudaErrorPeerAccessAlreadyEnabled) {
        ...
        WARN("failed to peer with device %d: %d %s", map->cudaDev, err,
    ...
    if (!map->sameProcess)
      NCCLCHECK(netMapShm(comm, map->mems + NCCL_NET_MAP_HOSTMEM));
    if (map->mems[NCCL_NET_MAP_DEVMEM].size) {
      map->mems[NCCL_NET_MAP_DEVMEM].gpuPtr = NULL;
      NCCLCHECK(ncclP2pImportShareableBuffer(
          comm, send->proxyConn.rank, map->mems[NCCL_NET_MAP_DEVMEM].size,
    ...
    if (map->mems[NCCL_NET_MAP_SHARED_DEVMEM].size) {
      void **sharedDevMemPtr =
          comm->proxyState->sharedDevMems + send->proxyConn.tpLocalRank;
      if (*sharedDevMemPtr == NULL) {
        map->mems[NCCL_NET_MAP_SHARED_DEVMEM].gpuPtr = NULL;
        NCCLCHECK(ncclP2pImportShareableBuffer(
            comm, send->proxyConn.rank,
            map->mems[NCCL_NET_MAP_SHARED_DEVMEM].size,
            &map->mems[NCCL_NET_MAP_SHARED_DEVMEM].ipcDesc, sharedDevMemPtr));
      }
  ...
  struct ncclSendMem *sendMem =
      (struct ncclSendMem *)NCCL_NET_MAP_GET_POINTER(map, gpu, sendMem);
  void *gdcMem = map->mems[NCCL_NET_MAP_GDCMEM].gpuPtr;
  send->conn.head = gdcMem ? (uint64_t *)gdcMem : &sendMem->head;

  struct ncclRecvMem *recvMem =
      (struct ncclRecvMem *)NCCL_NET_MAP_GET_POINTER(map, gpu, recvMem);
  send->conn.tail = &recvMem->tail;
  send->conn.stepSize = comm->buffSizes[NCCL_PROTO_SIMPLE] / NCCL_STEPS;
  send->conn.connFifo = recvMem->connFifo;
  struct ncclRecvMem *recvMemCpu =
      (struct ncclRecvMem *)NCCL_NET_MAP_GET_POINTER(map, cpu, recvMem);
  for (int i = 0; i < NCCL_STEPS; i++) {
    recvMemCpu->connFifo[i].offset = -1;
    recvMemCpu->connFifo[i].mode =
        map->shared ? NCCL_MODE_OFFSET : NCCL_MODE_NORMAL;
  }
...
static ncclResult_t recvConnect(struct ncclComm *comm,
                                struct ncclConnect *connectInfo, int nranks,
                                int rank, struct ncclConnector *recv) {
  struct connectMap *map = (connectMap *)recv->transportResources;
  void *opId;
  int sendUseGdr;

  memcpy(&sendUseGdr, (uint8_t *)connectInfo + sizeof(ncclNetHandle_t),
         sizeof(int));
  if (!sendUseGdr)
    recv->conn.flags &= ~NCCL_DIRECT_NIC;
  ...
  if (!map) {
    ...
    opId = recv;
    INFO(NCCL_PROXY,
         "recvConnect ncclProxyCallAsync opId=%p &recv->proxyConn=%p "
         "connectInfo=%p",
         opId, &recv->proxyConn, connectInfo);
    netRecvConnectArgs args = {0};
    args.proxyRank = *((int *)connectInfo);
    NCCLCHECK(ncclProxyCallAsync(comm, &recv->proxyConn, ncclProxyMsgConnect,
                                 &args, sizeof(netRecvConnectArgs),
                                 sizeof(struct connectMap), opId));
  } else {
    opId = recv;
  }
...
static ncclResult_t sendFree(struct ncclConnector *send) {
  ...
  if (map) {
    ...
    if (map->cudaDev != cudaDev && map->mems[NCCL_NET_MAP_DEVMEM].size) {
      if (ncclCuMemEnable()) {
        // cuMem API support
        NCCLCHECK(ncclP2pFreeShareableBuffer(
            &map->mems[NCCL_NET_MAP_DEVMEM].ipcDesc));
...
static ncclResult_t recvFree(struct ncclConnector *recv) {
  if (recv->transportResources)
    free(recv->transportResources);
  return ncclSuccess;
}

#define NCCL_SHARED_STEPS 16
static ncclResult_t sharedNetBuffersInit(struct ncclProxyState *proxyState,
                                         int cuda, int tpLocalRank, int type,
                                         int sameProcess, int nChannels,
                                         char **gpuPtr, char **cpuPtr,
                                         int *size, ncclIpcDesc *ipcDesc) {
  ...
  if (progressState->localPeers == NULL) {
    NCCLCHECK(
        ncclCalloc(&progressState->localPeers, proxyState->tpLocalnRanks));
  }
  ...
  if (state->size == 0) {
    state->size =
        (int64_t)nChannels * NCCL_SHARED_STEPS * proxyState->p2pChunkSize;
  }
  ...
  if (cuda && state->cudaBuff == NULL) {
    if (sameProcess == 0 || ncclCuMemEnable()) {
      NCCLCHECK(ncclP2pAllocateShareableBuffer(state->size, 0, &state->ipcDesc,
                                               (void **)&state->cudaBuff));
    } else {
#if defined(HIP_UNCACHED_MEMORY)
#if defined(HIP_CONTIGUOUS_MEMORY)
      NCCLCHECK(ncclCudaCalloc(&state->cudaBuff, state->size, nullptr,
                               cuda ? (rcclParamNetContiguousMem()
                                           ? hipDeviceMallocContiguous
                                           : hipDeviceMallocUncached)
                                    : hipDeviceMallocDefault));
#else
      NCCLCHECK(ncclCudaCalloc(&state->cudaBuff, state->size, nullptr,
                               cuda ? hipDeviceMallocUncached
                                    : hipDeviceMallocDefault));
#endif
#else
      NCCLCHECK(ncclCudaCalloc(&state->cudaBuff, state->size, nullptr,
                               cuda ? hipDeviceMallocFinegrained
                                    : hipDeviceMallocDefault));
#endif
    }
  }
  ...
  if (ipcDesc)
    memcpy(ipcDesc, &state->ipcDesc, sizeof(state->ipcDesc));
  return ncclSuccess;
}

static ncclResult_t sharedBuffersGet(struct ncclProxyState *proxyState,
                                     int channel, int slot, int *offset,
                                     size_t *size) {
  // Use different pools for different channels and also separate send/recv.
  int globalSlot = (channel * NCCL_SHARED_STEPS) + slot;
  *offset = proxyState->p2pChunkSize * globalSlot;
  if (size)
    *size = proxyState->p2pChunkSize;
  return ncclSuccess;
}

static ncclResult_t
sharedNetBuffersDestroy(struct ncclProxyState *proxyState, int tpLocalRank,
                        int type, struct ncclProxyConnection *connection) {
  if (proxyState->progressState.localPeers == NULL)
    NCCLCHECK(ncclInternalError);
  struct ncclProxyPeer *peer =
      proxyState->progressState.localPeers[tpLocalRank];
  if (peer == NULL)
    NCCLCHECK(ncclInternalError);
  struct ncclProxySharedP2p *state = type == 0 ? &peer->send : &peer->recv;
  if (state->size == 0)
...
static ncclResult_t proxySharedInit(struct ncclProxyConnection *connection,
  ...
  NCCLCHECK(sharedNetBuffersInit(
      proxyState, is_wsl2 == 0 ? 1 : 0, connection->tpLocalRank, 0,
      connection->sameProcess, nChannels, NULL, NULL, NULL, NULL));
  return ncclSuccess;
}

static ncclResult_t sendProxySetup(struct ncclProxyConnection *connection,
                                   struct ncclProxyState *proxyState,
                                   void *reqBuff, int reqSize, void *respBuff,
                                   int respSize, int *done) {
  struct setupReq *req = (struct setupReq *)reqBuff;
  if (reqSize != sizeof(struct setupReq))
    return ncclInternalError;
  ...
  /* DMA-BUF support */
  resources->useDmaBuf = resources->useGdr && proxyState->dmaBufSupport &&
  ...
  if ((resources->maxP2pBytes <= 0) ||
      (resources->maxP2pBytes > NCCL_MAX_NET_SIZE_BYTES)) {
    WARN(
        "sendProxySetup: net plugin returned invalid value for maxP2pBytes %ld \
      [allowed range: %ld - %ld] \n",
...
static ncclResult_t recvProxySetup(struct ncclProxyConnection *connection,
                                   struct ncclProxyState *proxyState,
                                   void *reqBuff, int reqSize, void *respBuff,
                                   int respSize, int *done) {
  struct setupReq *req = (struct setupReq *)reqBuff;
  ...
  if ((resources->maxP2pBytes <= 0) ||
      (resources->maxP2pBytes > NCCL_MAX_NET_SIZE_BYTES)) {
    WARN(
        "recvProxySetup: net plugin returned invalid value for maxP2pBytes %ld \
      [allowed range: %ld - %ld] \n",
        resources->maxP2pBytes, 0L, NCCL_MAX_NET_SIZE_BYTES);
    return ncclInternalError;
  }
  ...
  if (respSize != sizeof(ncclNetHandle_t))
    return ncclInternalError;
  NCCLCHECK(proxyState->ncclNet->listen(req->netDev, respBuff,
...
static ncclResult_t ncclNetGetDeviceHandle(ncclNetDeviceType type, int version,
                                           bool isRecv,
...
static ncclResult_t sendProxyConnect(struct ncclProxyConnection *connection,
                                     struct ncclProxyState *proxyState,
                                     void *reqBuff, int reqSize, void *respBuff,
                                     int respSize, int *done) {
  struct sendNetResources *resources =
      (struct sendNetResources *)(connection->transportResources);
  if (reqSize != sizeof(netSendConnectArgs))
    return ncclInternalError;
  ncclResult_t ret = ncclSuccess;
  netSendConnectArgs *req = (netSendConnectArgs *)reqBuff;
  NCCLCHECK(ncclNetGetDeviceHandle(
      resources->netDeviceType, resources->netDeviceVersion, false /*isRecv*/,
  ...
  if (resources->shared) {
    ...
    connection->proxyAppendPtr =
        localPeers[resources->tpLocalRank]->send.proxyAppend +
        resources->channelId;

    if (resources->maxRecvs > 1 && ncclParamNetSharedComms()) {
      // Connect or reuse connection for a netdev/remote rank.
      if (progressState->netComms[resources->netDev] == NULL) {
        NCCLCHECK(ncclCalloc(progressState->netComms + resources->netDev,
                             proxyState->tpnRanks));
      }
      struct ncclSharedNetComms *comms =
          progressState->netComms[resources->netDev] + resources->tpRemoteRank;
      if (comms->sendComm[resources->channelId] == NULL) {
        if (rccl_anp) {
          ret = proxyState->ncclNet->connect(
              resources->netDev, req->handle,
              comms->sendComm + resources->channelId,
              (ncclNetDeviceHandle_t **)(uintptr_t)(resources->channelId));
        } else {
          ret = proxyState->ncclNet->connect(resources->netDev, req->handle,
                                             comms->sendComm +
                                                 resources->channelId,
                                             &resources->netDeviceHandle);
        }
      ...
      if (rccl_anp) {
        ret = proxyState->ncclNet->connect(
            resources->netDev, req->handle, &resources->netSendComm,
            (ncclNetDeviceHandle_t **)(uintptr_t)(resources->channelId));
      } else {
        ret = proxyState->ncclNet->connect(resources->netDev, req->handle,
  ...
  *done = 1;
  ...
  if (resources->netDeviceHandle) {
    ...
    connection->needsProxyProgress =
        connection->netDeviceHandle->needsProxyProgress;
  } else {
  ...
  if (resources->shared ==
      0) { // Only allocate dedicated buffers for ring/tree, not for p2p
    for (int p = 0; p < NCCL_NUM_PROTOCOLS; p++) {
      NCCL_NET_MAP_ADD_POINTER(map, 0, p != NCCL_PROTO_LL && resources->useGdr,
                               proxyState->buffSizes[p], buffs[p]);
      resources->buffSizes[p] = proxyState->buffSizes[p];
    }
  } else {
    // Get shared buffers
    int bank = resources->useGdr ? NCCL_NET_MAP_SHARED_DEVMEM
                                 : NCCL_NET_MAP_SHARED_HOSTMEM;
    struct connectMapMem *mapMem = map->mems + bank;
    NCCLCHECK(sharedNetBuffersInit(
        proxyState, resources->useGdr, resources->tpLocalRank, 0,
        map->sameProcess, proxyState->p2pnChannels, &mapMem->gpuPtr,
        &mapMem->cpuPtr, &mapMem->size, &mapMem->ipcDesc));
    resources->buffSizes[NCCL_PROTO_SIMPLE] = mapMem->size;

    if (proxyState->allocP2pNetLLBuffers) {
      NCCL_NET_MAP_ADD_POINTER(map, 0, 0 /*p == NCCL_PROTO_LL*/,
                               proxyState->buffSizes[NCCL_PROTO_LL],
                               buffs[NCCL_PROTO_LL]);
      resources->buffSizes[NCCL_PROTO_LL] =
          proxyState->buffSizes[NCCL_PROTO_LL];
    }

    NCCL_NET_MAP_ADD_POINTER(map, 1, resources->useGdr, mapMem->size,
                             buffs[NCCL_PROTO_SIMPLE]);
  }
    ...
    if (resources->shared == 0) {
      if (!map->sameProcess || ncclCuMemEnable()) {
        ...
        NCCLCHECK(ncclP2pAllocateShareableBuffer(
            map->mems[NCCL_NET_MAP_DEVMEM].size, 0,
            &map->mems[NCCL_NET_MAP_DEVMEM].ipcDesc,
            (void **)&map->mems[NCCL_NET_MAP_DEVMEM].gpuPtr));
      } else {
#if defined(HIP_UNCACHED_MEMORY)
        ...
        NCCLCHECK(ncclCudaCalloc(&map->mems[NCCL_NET_MAP_DEVMEM].gpuPtr,
                                 map->mems[NCCL_NET_MAP_DEVMEM].size, nullptr,
                                 resources->useGdr
                                     ? (rcclParamNetContiguousMem()
                                            ? hipDeviceMallocContiguous
                                            : hipDeviceMallocUncached)
                                     : hipDeviceMallocDefault));
#else
        NCCLCHECK(ncclCudaCalloc(&map->mems[NCCL_NET_MAP_DEVMEM].gpuPtr,
                                 map->mems[NCCL_NET_MAP_DEVMEM].size, nullptr,
                                 resources->useGdr ? hipDeviceMallocUncached
                                                   : hipDeviceMallocDefault));
#endif
#else
        NCCLCHECK(ncclCudaCalloc(&map->mems[NCCL_NET_MAP_DEVMEM].gpuPtr,
                                 map->mems[NCCL_NET_MAP_DEVMEM].size, nullptr,
                                 resources->useGdr ? hipDeviceMallocFinegrained
                                                   : hipDeviceMallocDefault));
#endif
      }
      map->mems[NCCL_NET_MAP_DEVMEM].cpuPtr =
          map->mems[NCCL_NET_MAP_DEVMEM].gpuPtr;
    }
  }
  if (map->sameProcess) {
    NCCLCHECK(ncclCudaHostCalloc(&map->mems[NCCL_NET_MAP_HOSTMEM].cpuPtr,
                                 map->mems[NCCL_NET_MAP_HOSTMEM].size));
  ...
  if (ncclGdrCopy && map->sameProcess && ncclParamGdrCopySyncEnable()) {
    ...
    NCCLCHECK(
        ncclGdrCudaCalloc(&cpuPtr, &gpuPtr, 1, &resources->gdrDesc, nullptr));
  ...
  resources->sendMem =
      (struct ncclSendMem *)NCCL_NET_MAP_GET_POINTER(map, cpu, sendMem);
  resources->recvMem =
      (struct ncclRecvMem *)NCCL_NET_MAP_GET_POINTER(map, cpu, recvMem);
    ...
    if (resources->buffers[p]) {
      ...
      if (type == NCCL_PTR_CUDA && resources->useDmaBuf) {
        ...
        CUCHECK(cuMemGetHandleForAddressRange(
            (void *)&dmabuf_fd, (CUdeviceptr)resources->buffers[p],
            resources->buffSizes[p], CU_MEM_RANGE_HANDLE_TYPE_DMA_BUF_FD, 0));
        NCCLCHECK(proxyState->ncclNet->regMrDmaBuf(
            resources->netSendComm, resources->buffers[p],
      ...
      int type =
          NCCL_NET_MAP_DEV_MEM(map, buffs[p]) ? NCCL_PTR_CUDA : NCCL_PTR_HOST;
      if (type == NCCL_PTR_CUDA && proxyState->dmaBufSupport &&
          pfn_hsa_amd_portable_export_dmabuf) {
        int dmabuf_fd;
        uint64_t offset;
        CUCHECK(hsa_amd_portable_export_dmabuf(
            (const void *)resources->buffers[p], resources->buffSizes[p],
            &dmabuf_fd, &offset));
        NCCLCHECK(proxyState->ncclNet->regMrDmaBuf(
            resources->netSendComm, resources->buffers[p],
        ...
        TRACE(NCCL_INIT | NCCL_NET,
              "hsa_amd_portable_export_dmabuf buffer %p size %d handle %x "
              "offset %ld",
              (const void *)resources->buffers[p], resources->buffSizes[p],
              dmabuf_fd, offset);
      } else // FALL-THROUGH to nv_peermem GDR path
      ...
      if (resources->netDeviceHandle && proxyState->ncclNet->getDeviceMr)
        NCCLCHECK(proxyState->ncclNet->getDeviceMr(resources->netSendComm,
                                                   resources->mhandles[p],
...
static ncclResult_t recvProxyConnect(struct ncclProxyConnection *connection,
                                     struct ncclProxyState *proxyState,
                                     void *reqBuff, int reqSize, void *respBuff,
                                     int respSize, int *done) {
  if (reqSize != sizeof(netRecvConnectArgs))
    return ncclInternalError;
  struct recvNetResources *resources =
  ...
  bool rccl_anp = !(strcmp(proxyState->ncclNet->name, RCCL_ANP_PLUGIN_STR));
  NCCLCHECK(ncclNetGetDeviceHandle(resources->netDeviceType,
                                   resources->netDeviceVersion, true /*isRecv*/,
                                   &resources->netDeviceHandle));
  ...
  if (resources->shared) {
    ...
    connection->proxyAppendPtr =
        localPeers[resources->tpLocalRank]->recv.proxyAppend +
        resources->channelId;
    ...
    if (resources->maxRecvs > 1 && ncclParamNetSharedComms()) {
      ...
      struct ncclSharedNetComms *comms =
          progressState->netComms[resources->netDev] +
          resources->tpRemoteProxyRank;
      if (comms->recvComm[resources->channelId] == NULL) {
        if (rccl_anp) {
          ret = proxyState->ncclNet->accept(
              resources->netListenComm, comms->recvComm + resources->channelId,
              (ncclNetDeviceHandle_t **)(uintptr_t)(resources->channelId));
        } else {
          ret = proxyState->ncclNet->accept(
              resources->netListenComm, comms->recvComm + resources->channelId,
  ...
  *done = 1;
  ...
  if (map->sameProcess == 0)
    return ncclInternalError; // We don't support remote proxy for recv
  map->shared = resources->shared;

  if (resources->shared ==
      0) { // Only allocate dedicated buffers for ring/tree, not for p2p
    for (int p = 0; p < NCCL_NUM_PROTOCOLS; p++) {
      NCCL_NET_MAP_ADD_POINTER(map, 0, resources->useGdr,
                               proxyState->buffSizes[p], buffs[p]);
      resources->buffSizes[p] = proxyState->buffSizes[p];
    }
  } else {
    ...
    NCCLCHECK(sharedNetBuffersInit(proxyState, resources->useGdr,
                                   resources->tpLocalRank, 1, 1,
                                   proxyState->p2pnChannels, &mapMem->gpuPtr,
                                   &mapMem->cpuPtr, &mapMem->size, NULL));
    resources->buffSizes[NCCL_PROTO_SIMPLE] = mapMem->size;
    NCCL_NET_MAP_ADD_POINTER(map, 1, resources->useGdr, mapMem->size,
                             buffs[NCCL_PROTO_SIMPLE]);
  }
  ...
  if (proxyState->allocP2pNetLLBuffers) {
    NCCL_NET_MAP_ADD_POINTER(map, 0, 0 /*resources->useGdr*/,
                             proxyState->buffSizes[NCCL_PROTO_LL],
                             buffs[NCCL_PROTO_LL]);
  ...
  if (ncclGdrCopy && map->sameProcess) {
    uint64_t *cpuPtr, *gpuPtr;
    NCCLCHECK(
        ncclGdrCudaCalloc(&cpuPtr, &gpuPtr, 2, &resources->gdrDesc, nullptr));
    ...
    if (ncclParamGdrCopyFlushEnable())
      resources->gdcFlush = cpuPtr + 1;
  }
    ...
    if (resources->buffers[p]) {
      ...
      if (type == NCCL_PTR_CUDA && resources->useDmaBuf) {
        ...
        CUCHECK(cuMemGetHandleForAddressRange(
            ...
            resources->buffSizes[p], CU_MEM_RANGE_HANDLE_TYPE_DMA_BUF_FD, 0));
        NCCLCHECK(proxyState->ncclNet->regMrDmaBuf(
            resources->netRecvComm, resources->buffers[p],
            resources->buffSizes[p], type, 0ULL, dmabuf_fd,
            &resources->mhandles[p]));
        (void)close(dmabuf_fd);
      } else // FALL-THROUGH to nv_peermem GDR path
      ...
      if (resources->netDeviceType != NCCL_NET_DEVICE_HOST &&
          proxyState->ncclNet->getDeviceMr)
        NCCLCHECK(proxyState->ncclNet->getDeviceMr(resources->netRecvComm,
                                                   resources->mhandles[p],
                                                   &connection->mhandles[p]));
    }
  }
...
static ncclResult_t sendProxyFree(struct ncclProxyConnection *connection,
                                  struct ncclProxyState *proxyState) {
  struct sendNetResources *resources =
      (struct sendNetResources *)(connection->transportResources);
  if (connection->state == connSharedInitialized) { // NVB Preconnect
    NCCLCHECK(sharedNetBuffersDestroy(proxyState, connection->tpLocalRank, 0,
                                      connection));
    return ncclSuccess;
  }
  ...
  if (connection->state == connConnected) {
    for (int p = 0; p < NCCL_NUM_PROTOCOLS; p++) {
      if (resources->buffers[p]) {
        NCCLCHECK(proxyState->ncclNet->deregMr(resources->netSendComm,
                                               resources->mhandles[p]));
      }
    }
    ...
    if (!resources->map.sameProcess || ncclCuMemEnable()) {
      ...
      if (mems[NCCL_NET_MAP_DEVMEM].size) {
        NCCLCHECK(
            ncclP2pFreeShareableBuffer(&mems[NCCL_NET_MAP_DEVMEM].ipcDesc));
      }
    }
    if (mems[NCCL_NET_MAP_GDCMEM].cpuPtr)
      NCCLCHECK(ncclGdrCudaFree(resources->gdrDesc));
    if (resources->shared) {
      NCCLCHECK(sharedNetBuffersDestroy(proxyState, resources->tpLocalRank, 0,
                                        connection));
      if (resources->maxRecvs > 1 && ncclParamNetSharedComms()) {
        struct ncclSharedNetComms *comms =
            proxyState->progressState.netComms[resources->netDev] +
            resources->tpRemoteRank;
        comms->sendRefCount[resources->channelId]--;
        if (comms->sendRefCount[resources->channelId] == 0)
          NCCLCHECK(proxyState->ncclNet->closeSend(
              comms->sendComm[resources->channelId]));
      } else {
...
static ncclResult_t recvProxyFree(struct ncclProxyConnection *connection,
                                  struct ncclProxyState *proxyState) {
  struct recvNetResources *resources =
      (struct recvNetResources *)(connection->transportResources);
  if (connection->state == connSharedInitialized) { // NVB Preconnect
    NCCLCHECK(sharedNetBuffersDestroy(proxyState, connection->tpLocalRank, 1,
                                      connection));
    return ncclSuccess;
  }
  ...
  if (connection->state == connConnected) {
    for (int p = 0; p < NCCL_NUM_PROTOCOLS; p++) {
      if (resources->buffers[p]) {
        NCCLCHECK(proxyState->ncclNet->deregMr(resources->netRecvComm,
                                               resources->mhandles[p]));
      }
    }
    struct connectMapMem *mems = resources->map.mems;
    ...
    if (mems[NCCL_NET_MAP_GDCMEM].cpuPtr)
      NCCLCHECK(ncclGdrCudaFree(resources->gdrDesc));
    if (resources->shared) {
      NCCLCHECK(sharedNetBuffersDestroy(proxyState, resources->tpLocalRank, 1,
                                        connection));
      if (resources->maxRecvs > 1 && ncclParamNetSharedComms()) {
        struct ncclSharedNetComms *comms =
            proxyState->progressState.netComms[resources->netDev] +
        ...
        if (comms->recvRefCount[resources->channelId] == 0)
          NCCLCHECK(proxyState->ncclNet->closeRecv(
              comms->recvComm[resources->channelId]));
      } else {
...
static_assert(NCCL_STEPS <= NCCL_NET_MAX_REQUESTS,
              "Not enough net requests to cover for steps");
...
static ncclResult_t sendProxyProgress(struct ncclProxyState *proxyState,
  ...
  if (args->state == ncclProxyOpReady) {
    for (int s = 0; s < args->nsubs; s++) {
      ...
      addNewProxyOp(proxyState, facebook::rcclx::ProxyOpType::SEND, sub);
      if (!sub->reg)
  ...
  if (args->state == ncclProxyOpProgress) {
    ...
    for (int s = 0; s < args->nsubs; s++) {
      ...
      char *localBuff =
          NCCL_NET_MAP_GET_POINTER(&resources->map, cpu, buffs[p]);
      ...
      if (sub->posted < sub->nsteps && sub->posted < sub->done + maxDepth) {
        ...
        if (resources->shared) {
          if (!sub->reg) {
            ...
            NCCLCHECK(sharedBuffersGet(proxyState, sub->channelId,
                                       sharedBuffSlot * args->nsubs + s,
                                       &offset, NULL));
            __atomic_store_n(&resources->recvMem->connFifo[buffSlot].offset,
                             offset, __ATOMIC_RELAXED);
            __sync_synchronize();
          }
        ...
        ncclProfilerRecordProxyOpEventState(s, args, sub->posted,
                                            sub->transSize,
                                            ncclProfilerProxyOpSendPosted);
        ncclProfilerRecordProxyStepEventState(s, args, postedStepId,
                                              ncclProfilerProxyStepSendGPUWait);
        updateProxyOpCounter(proxyState->proxyTrace, sub->traceKey,
  ...
  if (sub->transmitted < sub->posted &&
    ...
    if (connFifo[buffSlot].size != -1 &&
        (*recvTail > tail || p == NCCL_PROTO_LL)) {
      // We have something to receive, let's check if it's completely ready.
      int size = connFifo[buffSlot].size;
#if defined(ENABLE_NPKIT) && defined(ENABLE_NPKIT_EVENT_NET_SEND_ENTRY) &&     \
    defined(ENABLE_NPKIT_EVENT_NET_SEND_EXIT)
      ...
      if (p == NCCL_PROTO_LL128) {
        ...
        if (!ready) {
          // When data is in sysmem, we need to wait until all flags are
          // correct since the GPU only called threadfence()
          uint64_t flag = sub->base + sub->transmitted + 1;
          int nFifoLines = DIVUP(connFifo[buffSlot].size,
                                 sizeof(uint64_t) * NCCL_LL128_LINEELEMS);
          volatile uint64_t *lines = (volatile uint64_t *)buff;
          ready = 1;
          for (int i = 0; i < nFifoLines; i++) {
            if (lines[i * NCCL_LL128_LINEELEMS + NCCL_LL128_DATAELEMS] !=
                flag) {
              ready = 0;
              break;
            }
        ...
        if (resources->shared) {
          buff =
              sub->reg
                  ? (char *)sub->sendbuff + sub->transmitted * NCCL_MAX_NET_SIZE
                  : localBuff + resources->recvMem->connFifo[buffSlot].offset;
        } else if (sub->reg) {
          size_t sendSize;
          sub->ringAlgo->getNextSendAddr(sub->transmitted, (uint8_t **)&buff,
                                         &sendSize, &sub->sendMhandle);
      ...
      if (ready) {
        ...
        ncclProfilerRecordProxyOpEventState(
            s, args, sub->transmitted + args->sliceSteps, sub->transSize,
        ...
        // Coverity complains about the size here as pointing to an
        // out-of-scope temporary.  Which is nonsense, since size is a plain
        // integer. coverity[use_invalid:FALSE]
        NCCLCHECK(proxyState->ncclNet->isend(
            resources->netSendComm, buff, size, resources->tpRank,
    ...
    defined(ENABLE_NPKIT_EVENT_NET_SEND_EXIT)
          ...
          TRACE(NCCL_NET,
                "sendProxy [%ld/%d/%d] Isend posted, req %p, buff %p, size "
                "%d, proto %d, myRank %d, channelId %d, mhandle %p",
                sub->transmitted, buffSlot, sub->nsteps,
                sub->requests[buffSlot], buff, size, p, proxyState->tpRank,
          ...
          ncclProfilerRecordProxyOpEventState(
              s, args, sub->transmitted, sub->transSize,
              ncclProfilerProxyOpSendTransmitted);
          ncclProfilerRecordProxyStepEventState(s, args, transmittedStepId,
  ...
  if (sub->done < sub->transmitted) {
...
#if defined(ENABLE_NPKIT) && defined(ENABLE_NPKIT_EVENT_NET_SEND_ENTRY) &&     \
    defined(ENABLE_NPKIT_EVENT_NET_SEND_EXIT)
      NpKit::CollectCpuEvent(NPKIT_EVENT_NET_SEND_EXIT,
#if defined(ENABLE_NPKIT_NET_COLLECT_POLL_CNT)
                             g_npkit_net_poll_cnt,
#else
                             sub->npKitSizesFifo[buffSlot],
#endif
                             uint64_t(sub->requests + buffSlot) /
                                 sizeof(void *),
...
#endif
#endif
#if defined(ENABLE_NPKIT) && defined(ENABLE_NPKIT_EVENT_NET_TEST_ENTRY) &&     \
    defined(ENABLE_NPKIT_EVENT_NET_TEST_EXIT)
      NpKit::CollectCpuEvent(NPKIT_EVENT_NET_TEST_ENTRY,
#if defined(ENABLE_NPKIT_NET_COLLECT_POLL_CNT)
                             g_npkit_net_poll_cnt,
#else
                             sub->npKitSizesFifo[buffSlot],
    ...
    defined(ENABLE_NPKIT_EVENT_NET_TEST_EXIT)
      ...
      __sync_synchronize();
      TRACE(NCCL_NET, "sendProxy [%ld/%d/%d] request %p done", sub->done,
            buffSlot, sub->nsteps, sub->requests[buffSlot]);
      sub->done += args->sliceSteps;
      ncclProfilerStopProxyStepEvent(s, args, doneStepId);
      ncclProfilerRecordProxyOpEventState(s, args, sub->done, sub->transSize,
  ...
  if (args->state == ncclProxyOpReady) {
    ...
    for (int s = 0; s < args->nsubs; s++) {
      ...
      if (groupSize == maxRecvs) {
        ...
        for (next = s; next < args->nsubs; next++) {
          struct recvNetResources *nextRes =
              (struct recvNetResources *)(args->subs[next]
                                              .connection->transportResources);
          if (nextRes->netRecvComm == recvComm)
            break;
        }
        if (next == args->nsubs) { // Not found
          groupSize = 0;
        } else if (s != next) { // We found a sub later with the same recvComm ;
                                // swap subs
      ...
      for (int i = 0; i < groupSize; i++)
        sub[-i].groupSize = groupSize;
      ncclProfilerStartRecvProxyOpEvent(s, args);
      facebook_rccl::addNewProxyOp(proxyState->proxyTrace, sub);
      if (!sub->reg)
  ...
  if (args->state == ncclProxyOpProgress) {
    ...
    for (int s = 0; s < args->nsubs; s += args->subs[s].groupSize) {
      ...
      for (int i = 0; i < subGroup->groupSize; i++) {
        ...
        if (sub->posted < sub->nsteps) {
          ...
          if (p == NCCL_PROTO_SIMPLE) {
            if (resources->shared) {
              if (sub->reg) {
                // Wait until CUDA kernel has started before we access the user
                // buffer directly.
                ...
                ptrs[subCount] =
                    sub->recvbuff + sub->posted * NCCL_MAX_NET_SIZE;
                sizes[subCount] = std::min(
                    NCCL_MAX_NET_SIZE,
                    (ssize_t)(sub->nbytes - sub->posted * NCCL_MAX_NET_SIZE));
              } else {
                int sharedBuffSlot = sub->posted % maxDepth;
                int offset;
                NCCLCHECK(sharedBuffersGet(proxyState, sub->channelId,
                                           sharedBuffSlot * args->nsubs + s + i,
                                           &offset, sizes + subCount));
                __atomic_store_n(&connFifo[buffSlot].offset, offset,
                                 __ATOMIC_RELAXED);
              ...
              if (sub->reg) {
                if (!sub->regBufferReady &&
                  ...
                  continue;
                sub->regBufferReady = 1;
                sub->ringAlgo->getNextRecvAddr(
                    sub->posted, (uint8_t **)&ptrs[subCount], &sizes[subCount],
                    &sub->recvMhandle);
              } else {
      ...
      if (subCount) {
        ...
        struct recvNetResources *resources =
            (struct recvNetResources *)(subGroup->connection
                                            ->transportResources);
        void **requestPtr = subGroup->requests + (step % NCCL_STEPS);
        bool ignoreCompletion = ncclParamNetOptionalRecvCompletion() &&
                                ((args->protocol == NCCL_PROTO_LL128) ||
                                 (args->protocol == NCCL_PROTO_LL)) &&
                                (subCount == 1);
        if (ignoreCompletion)
          *requestPtr = (void *)NCCL_NET_OPTIONAL_RECV_COMPLETION;
        NCCLCHECK(proxyState->ncclNet->irecv(resources->netRecvComm, subCount,
                                             ptrs, sizes, tags, mhandles,
                                             requestPtr));
        if (*requestPtr) {
          ...
          for (int i = 0; i < subGroup->groupSize; i++) {
            ...
            int postedStepId = sub->posted;
            TRACE(NCCL_NET,
                  "recvProxy [%ld/%ld/%d] Irecv posted, buff %p, size %ld, "
                  "myRank %d, channelId %d, mhandle %p",
                  sub->posted, (sub->base + sub->posted) % NCCL_STEPS,
                  sub->nsteps, ptrs[i], sizes[i], proxyState->tpRank,
                  sub->channelId, mhandles[i]);

#if defined(ENABLE_NPKIT) && defined(ENABLE_NPKIT_EVENT_NET_RECV_ENTRY) &&     \
    defined(ENABLE_NPKIT_EVENT_NET_RECV_EXIT)
            NpKit::CollectCpuEvent(
            ...
            sub->posted += args->sliceSteps;
            ncclProfilerRecordProxyOpEventState(s + i, args, sub->posted,
                                                sub->transSize,
                                                ncclProfilerProxyOpRecvPosted);
            ncclProfilerRecordProxyStepEventState(
                s + i, args, postedStepId, ncclProfilerProxyStepRecvWait);
    ...
    for (int s = 0; s < args->nsubs; s += args->subs[s].groupSize) {
      ...
      if (subGroup->posted > subGroup->received) {
        ...
        void *mhandles[NCCL_PROXY_MAX_SUBS];
        for (int i = 0; i < NCCL_PROXY_MAX_SUBS; i++)
          sizes[i] = 0;
    ...
    defined(ENABLE_NPKIT_EVENT_NET_RECV_EXIT)
            ...
            struct recvNetResources *resources =
                (struct recvNetResources *)(sub->connection
                                                ->transportResources);
            volatile struct ncclConnFifo *connFifo =
                (volatile struct ncclConnFifo *)resources->recvMem->connFifo;
            connFifo[buffSlot].size = -1;
            ...
            ncclProfilerRecordProxyOpEventState(
                s + i, args, sub->received, sub->transSize,
                ncclProfilerProxyOpRecvReceived);
            ncclProfilerRecordProxyStepEventState(
                s + i, args, receivedStepId,
          ...
          if (totalSize > 0 && p == NCCL_PROTO_SIMPLE && needFlush) {
            ...
            if (rcclParamNetHdpFlush() && resources->curr_hdp_reg) {
              ...
              *resources->curr_hdp_reg = 0x1;
              ...
              if (once) {
                once = false;
                INFO(NCCL_INIT, "%s: flushed HDP %p", __func__,
            ...
            if (resources->gdcFlush) {
              ...
              for (int i = 0; i < subGroup->groupSize; i++) {
                ...
                if (step < sub->nsteps) {
                  ...
                  char *localBuff =
                      NCCL_NET_MAP_GET_POINTER(&resources->map, cpu, buffs[p]);
                  int buffSlot =
                      (sub->base + sub->received - args->sliceSteps) %
                      NCCL_STEPS;
                  if (resources->shared) {
                    ptrs[subCount] =
                        sub->reg
                            ? (char *)sub->recvbuff + step * NCCL_MAX_NET_SIZE
                            : localBuff +
                                  resources->recvMem->connFifo[buffSlot].offset;
                  } else {
                    if (sub->reg) {
                      sub->ringAlgo->getNextRecvAddr(
                          step, (uint8_t **)&ptrs[subCount], NULL,
                          &sub->recvMhandle);
                    } else {
                      ptrs[subCount] = localBuff + buffSlot * stepSize;
                    }
              ...
              NCCLCHECK(proxyState->ncclNet->iflush(
                  resources->netRecvComm, subCount, ptrs, sizes, mhandles,
      ...
      if (subGroup->received > subGroup->transmitted) {
        ...
        if (done) {
          for (int i = 0; i < subGroup->groupSize; i++) {
            ...
            sub->transmitted += args->sliceSteps;
            ncclProfilerRecordProxyOpEventState(
                s + i, args, sub->transmitted, sub->transSize,
                ncclProfilerProxyOpRecvTransmitted);
            ncclProfilerRecordProxyStepEventState(
                s + i, args, transmittedStepId,
      ...
      for (int i = 0; i < subGroup->groupSize; i++) {
        ...
        if (sub->transmitted > sub->done) {
          ...
          while (done > sub->base + sub->done &&
                 // LL and LL128 can acknowledge 0-bytes send before they even
                 // happen. Don't go past what we transmitted.
                 sub->transmitted > sub->done) {
            if (subGroup->recvRequestsCache[sub->done % NCCL_STEPS]) {
              // the multirecv requests are only cached in the first sub.
              if (proxyState->ncclNet->irecvConsumed)
                NCCLCHECK(proxyState->ncclNet->irecvConsumed(
                    resources->netRecvComm, subGroup->recvRequestsSubCount,
                    subGroup->recvRequestsCache[sub->done % NCCL_STEPS]));
              subGroup->recvRequestsCache[sub->done % NCCL_STEPS] = NULL;
            }
            sub->done += args->sliceSteps;
            ncclProfilerStopProxyStepEvent(s + i, args, doneStepId);
            ncclProfilerRecordProxyOpEventState(s + i, args, sub->done,
                                                sub->transSize,
...
ncclResult_t ncclNetDeregBuffer(struct ncclComm *comm,
                                struct ncclProxyConnector *proxyConn,
                                void *handle) {
  NCCLCHECK(ncclProxyCallBlocking(comm, proxyConn, ncclProxyMsgDeregister,
                                  &handle, sizeof(void *), NULL, 0));
  INFO(NCCL_REG, "rank %d - deregistered net buffer handle %p", comm->rank,
       handle);
  return ncclSuccess;
}

static ncclResult_t netRegisterBuffer(ncclComm *comm, const void *userbuff,
                                      size_t buffSize,
                                      struct ncclConnector **peerConns,
                                      int nPeers, struct ncclReg *regRecord,
      ...
      if (found) {
        *outRegBufFlag = 1;
        ...
        INFO(NCCL_REG,
             ...
             regRecord->pages * comm->regCache.pageSize, netHandle->handle);
      } else {
        struct netRegInfo info = {regRecord->addr,
                                  regRecord->pages * comm->regCache.pageSize};
        void *handle = NULL;

        if (peerConn->conn.flags & NCCL_DIRECT_NIC) {
          NCCLCHECKGOTO(ncclProxyCallBlocking(
                            comm, peerProxyConn, ncclProxyMsgRegister, &info,
...
fail:
  *outRegBufFlag = 0;
  WARN("rank %d failed to NET register userbuff %p buffSize %ld GDR flag %d",
       comm->rank, userbuff, buffSize, gdrFlag);
  goto exit;
}

ncclResult_t ncclNetLocalRegisterBuffer(ncclComm *comm, const void *userbuff,
                                        size_t buffSize,
                                        struct ncclConnector **peerConns,
  ...
  *outRegBufFlag = 0;
  if (comm && userbuff && buffSize > 0 && nPeers > 0) {
    ...
    if (isValid)
      NCCLCHECKGOTO(netRegisterBuffer(comm, userbuff, buffSize, peerConns,
                                      nPeers, regRecord, outRegBufFlag,
                                      outHandle),
                    ret, fail);
  }
...
static ncclResult_t cleanupNet(struct ncclComm *comm,
...
ncclResult_t ncclNetGraphRegisterBuffer(
    ncclComm *comm, const void *userbuff, size_t buffSize,
  ...
  *outRegBufFlag = 0;
  if (comm && userbuff && buffSize > 0 && nPeers > 0) {
    CUDACHECKGOTO(cuMemGetAddressRange((CUdeviceptr *)&baseSend, &baseSendSize,
                                       (CUdeviceptr)userbuff),
                  ret, fail);
    NCCLCHECKGOTO(ncclCommGraphRegister(comm, baseSend, baseSendSize,
                                        (void **)&regRecord),
                  ret, fail);
    NCCLCHECKGOTO(netRegisterBuffer(comm, userbuff, buffSize, peerConns, nPeers,
                                    regRecord, outRegBufFlag, outHandle),
                  ret, fail);
    if (*outRegBufFlag) {
...
static ncclResult_t sendProxyRegBuffer(struct ncclProxyConnection *connection,
                                       struct ncclProxyState *proxyState,
  ...
  if (resources->useDmaBuf) {
    int dmabuf_fd;
    CUCHECKGOTO(cuMemGetHandleForAddressRange(
                    (void *)&dmabuf_fd, (CUdeviceptr)info->buffer, info->size,
                    CU_MEM_RANGE_HANDLE_TYPE_DMA_BUF_FD, 0),
                ret, peermem);
    NCCLCHECKGOTO(proxyState->ncclNet->regMrDmaBuf(
                      resources->netSendComm, (void *)info->buffer, info->size,
...
#endif
  if (needReg) {
    NCCLCHECKGOTO(proxyState->ncclNet->regMr(resources->netSendComm,
                                             (void *)info->buffer, info->size,
...
static ncclResult_t recvProxyRegBuffer(struct ncclProxyConnection *connection,
                                       struct ncclProxyState *proxyState,
                                       void *reqBuff, int reqSize,
                                       void *respBuff, int respSize,
                                       int *done) {
  ...
  if (resources->useDmaBuf) {
    ...
    CUCHECKGOTO(cuMemGetHandleForAddressRange(
                ...
                ret, peermem);
    NCCLCHECKGOTO(proxyState->ncclNet->regMrDmaBuf(
                      resources->netRecvComm, (void *)info->buffer, info->size,
                      NCCL_PTR_CUDA, 0ULL, dmabuf_fd, &handle),
                  ret, peermem);
    (void)close(dmabuf_fd);
    needReg = false;
...
#endif
  if (needReg) {
    NCCLCHECKGOTO(proxyState->ncclNet->regMr(resources->netRecvComm,
                                             (void *)info->buffer, info->size,
                                             NCCL_PTR_CUDA, &handle),
                  ret, fail);
  }
...
static ncclResult_t sendProxyDeregBuffer(struct ncclProxyConnection *connection,
                                         struct ncclProxyState *proxyState,
                                         void *reqBuff, int reqSize,
                                         int *done) {
  void *handle;
...
struct ncclTransport netTransport = {
    "NET",
    canConnect,
    {sendSetup, sendConnect, sendFree, proxySharedInit, sendProxySetup,
     sendProxyConnect, sendProxyFree, sendProxyProgress, sendProxyRegBuffer,
     sendProxyDeregBuffer},
    {recvSetup, recvConnect, recvFree, proxySharedInit, recvProxySetup,
     recvProxyConnect, recvProxyFree, recvProxyProgress, recvProxyRegBuffer,
     recvProxyDeregBuffer}};
