...
struct RunWorkBatch<ncclFuncSendRecv, T, RedOp, NCCL_ALGO_RING, NCCL_PROTO_SIMPLE> {
  ...
  __device__  __attribute__((noinline)) void run() {
    ...
    if (isCopy) {
      ...
      reduceCopy<COLL_UNROLL*2, USE_ACC, RedOp, T, 0,1,1, 0,1,1, /*PreOpSrcs=*/0>
      ...
      if (work->sendProtoLL) {
        ...
        runSend<ProtoSimple<1,1,USE_ACC,8>>(subtid, subtn, group, work);
#elif defined(__gfx908__) || defined(__gfx942__) || defined(__gfx950__)
        runSend<ProtoSimple<1,1,USE_ACC,4>>(subtid, subtn, group, work);
#else
...
#if defined(__gfx90a__)
        runRecv<ProtoSimple<1,1,USE_ACC,8>>(subtid, subtn, group, work);
#elif defined(__gfx908__) || defined(__gfx942__) || defined(__gfx950__)
        runRecv<ProtoSimple<1,1,USE_ACC,4>>(subtid, subtn, group, work);
#else
