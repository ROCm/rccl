 ...
 *reserved. Modifications Copyright (c) Microsoft Corporation. Licensed under
 *the MIT License.
 *
...
#if CUDART_VERSION >= 9020 || defined(__HIP_PLATFORM_AMD__) ||                 \
    defined(__HIPCC__)
...
const char *ncclFuncStr[NCCL_NUM_FUNCTIONS + 2] = {
    "AllGather", "AllReduce",     "AllToAllPivot", "Broadcast",
    "Reduce",    "ReduceScatter", "SendRecv"};
const char *ncclAlgoStr[NCCL_NUM_ALGORITHMS] = {
    "Tree", "Ring", "CollNetDirect", "CollNetChain", "NVLS", "NVLSTree", "PAT"};
const char *ncclProtoStr[NCCL_NUM_PROTOCOLS] = {"LL", "LL128", "Simple"};
const char *ncclDevRedOpStr[ncclNumDevRedOps] = {"Sum", "Prod", "MinMax",
                                                 "PreMulSum", "SumPostDiv"};
const char *ncclTypeStr[ncclNumTypes] = {"_i8",  "_u8",  "_i32", "_u32",
                                         "_i64", "_u64", "_f16", "_f32",
                                         "_f64", "_b16"};
...
ncclResult_t commSetUnrollFactor(struct ncclComm *comm) {
  ...
  if (IsArchMatch(devProp.gcnArchName, "gfx950"))
    comm->unroll = NCCL_UNROLL_1;
           ...
           ((IsArchMatch(devProp.gcnArchName, "gfx942") &&
...
#ifdef ENABLE_MSCCLPP
size_t std::hash<ncclUniqueId>::operator()(
    const ncclUniqueId &uniqueId) const noexcept {
...
static ncclResult_t ncclInit() {
  ...
  if (strcmp(strValue, "1") == 0)
    WARN(
        "NUMA auto balancing enabled which can lead to variability in the RCCL "
        "performance! Disable by \"sudo sysctl kernel.numa_balancing=0\"");
  ...
  if (strstr(verStr, "cray") == NULL) {
    NCCLCHECK(ncclTopoGetStrFromSys("/sys/devices/virtual/dmi/id",
                                    "bios_version", strValue));
    if (strncmp("Hyper-V UEFI Release", strValue, 20) != 0) {
      ...
      if (strstr(strValue, "iommu=pt") == NULL)
        WARN("Missing \"iommu=pt\" from kernel command line which can lead to "
             "system instablity or hang!");
    }
#ifndef HIP_UNCACHED_MEMORY
    char *env = getenv("HSA_FORCE_FINE_GRAIN_PCIE");
    if (env == NULL || strcmp(env, "1") != 0)
      WARN("Missing \"HSA_FORCE_FINE_GRAIN_PCIE=1\" from environment which can "
           "lead to low RCCL performance, system instablity or hang!");
#endif
  }
  ...
  do {
    ...
    for (int channel = 0; channel < MAXCHANNELS; channel++) {
      ...
      for (int i = 0; i < count; i++) {
        volatile struct ncclCollTrace *td = comm->collTrace +
                                            COLLTRACE_NUM_ITEMS * channel +
        ...
        if (type == ncclCollTraceDataType) {
          sprintf(
              line,
              "## [%012.6f] [%02d:%02d-%02d:%02x] L:%04d DT %08x %016lx %016lx",
              (double)(td->timeStamp) / vega_gpu_rtc_freq, comm->rank, td->bid,
              td->channelId, td->tid, fIdx, td->data_0, td->opCount,
              td->data_1);
        } else {
          if (type & ncclCollTraceP2pElemType)
            sprintf(line, "## [%012.6f] [%02d:%02d-%02d:%02x] %06x-%06x",
                    (double)(td->timeStamp) / vega_gpu_rtc_freq, comm->rank,
                    td->bid, td->channelId, td->tid, td->p2pOpCount[0],
                    td->p2pOpCount[1]);
          else
            sprintf(line, "## [%012.6f] [%02d:%02d-%02d:%02x] %06lx",
                    (double)(td->timeStamp) / vega_gpu_rtc_freq, comm->rank,
                    td->bid, td->channelId, td->tid, td->opCount);
          offset = strlen(line);
          if (type == ncclCollTraceCollElemType) {
            sprintf(line + offset,
                    " CE %s nw %d bi %d nc %d root %d busId %lx nRanks %d",
                    funcNames[fIdx], td->coll.nWarps, td->coll.bid,
                    td->coll.nChannels, td->coll.root, comm->busId,
                    comm->nRanks);
          } else if (type == ncclCollTraceP2pElemType) {
            sprintf(line + offset,
                    " Recv %d -> %d/%d/%d/%d ConnIdx/LL/Reg/nc %d/%d/%d/%d -> "
                    "Send %d cb %d busId %lx nRanks %d",
                    td->p2p.recvRank, td->p2p.recvConnIndex,
                    td->p2p.recvProtoLL, td->p2p.recvRegistered,
                    td->p2p.nRecvChannels, td->p2p.sendConnIndex,
                    td->p2p.sendProtoLL, td->p2p.sendRegistered,
                    td->p2p.nSendChannels, td->p2p.sendRank,
                    td->p2p.channelBase, comm->busId, comm->nRanks);
          } else {
            switch (type & 0xf) {
            case ncclCollTraceKernelLaunchType:
            case ncclCollTraceCollLaunchType:
              if ((type & 0xf) == ncclCollTraceKernelLaunchType)
                sprintf(line + offset, " KL %s [%02d:%02d-%02d:%02x] HWID %8x ",
                        funcNames[fIdx], comm->rank, td->bid, td->channelId,
                        td->tid, td->data_0);
              else if ((type & 0xf) == ncclCollTraceCollLaunchType)
                sprintf(line + offset, " CL %s [%02d:%02d-%02d:%02x] %d ",
                        funcNames[fIdx], comm->rank, td->bid, td->channelId,
                        td->tid, td->batchIx);
              offset = strlen(line);
              if ((type & 0xf0) == ncclCollTraceCollElemType)
                sprintf(line + offset,
                        " nw %d bi %d nc %d root %d busId %lx nRanks %d",
                        td->coll.nWarps, td->coll.bid, td->coll.nChannels,
                        td->coll.root, comm->busId, comm->nRanks);
            ...
            case ncclCollTraceKernelEndType:
              sprintf(line + offset,
                      " KE %s [%02d:%02d-%02d:%02x] busId %lx nRanks %d",
                      funcNames[fIdx], comm->rank, td->bid, td->channelId,
                      td->tid, comm->busId, comm->nRanks);
              break;
            case ncclCollTraceAbortType:
              sprintf(line + offset, " KA %s [%02d:%02d-%02d:%02x]",
                      funcNames[fIdx], comm->rank, td->bid, td->channelId,
                      td->tid);
              break;
            default:
...
void ncclCommPushFree(struct ncclComm *comm, void *obj) {
  struct ncclDestructor *dtor =
      ncclMemoryStackAlloc<struct ncclDestructor>(&comm->memPermanent);
...
static ncclResult_t commFree(ncclComm_t comm) {
  ...
  /* in commReclaim, we have guaranteed only last rank which calls
   * ncclCommDestroy() will free all intra-process communicators; therefore, we
   * only need to focus on local resource cleanup in commFree(). */
  if (comm->proxyState && comm->proxyRefCountOld == 0 &&
      comm->proxyState->thread) {
    PTHREADCHECK(pthread_join(comm->proxyState->thread, nullptr),
                 "pthread_join");
    if (comm->proxyState->threadUDS) {
      // UDS support
      PTHREADCHECK(pthread_join(comm->proxyState->threadUDS, nullptr),
                   "pthread_join");
    }
  }
  ...
  prof = (struct ncclProf *)malloc(sizeof(struct ncclProf) * MAXCHANNELS *
                                   PROFILE_NUM_LAUNCHES);
  CUDACHECK(
      hipMemcpy(prof, comm->devComm->devProf,
                sizeof(struct ncclProf) * MAXCHANNELS * PROFILE_NUM_LAUNCHES,
                hipMemcpyDeviceToHost));
#define VEGA_GPU_RTC_FREQUENCY 2.5E7
  for (int i = 0; i < comm->nChannels; i++) {
    for (int s = 0; s < prof[MAXCHANNELS * i].seq; s++) {
      if (prof[MAXCHANNELS * s + i].count == 0)
        continue;
      for (int j = 0; j < prof[MAXCHANNELS * s + i].count; j++) {
        INFO(NCCL_INIT, "# [%02d:%02d] %02d-%02d L:%04u %6.2fus", comm->rank, i,
             s, j, prof[MAXCHANNELS * s + i].elem[j].line,
             (prof[MAXCHANNELS * s + i].elem[j].timeStamp -
              prof[MAXCHANNELS * s + i].elem[0].timeStamp) /
                 VEGA_GPU_RTC_FREQUENCY * 1.0E6);
      }
    }
  }
  free(prof);
  ...
  if (comm->nodeRanks) {
    for (int n = 0; n < comm->nNodes; n++)
      free(comm->nodeRanks[n].localRankToRank);
    free(comm->nodeRanks);
  }
  ...
  for (int channel = 0; channel < MAXCHANNELS; channel++)
    NCCLCHECK(freeChannel(comm->channels + channel, comm->nRanks, 1,
                          comm->localRanks));
  ...
  if (comm->sharedRes) {
    if (ncclAtomicRefCountDecrement(&comm->sharedRes->refCount) == 0) {
      for (int c = 0; c < MAXCHANNELS; c++) {
        if (comm->sharedRes->peers[c])
          free(comm->sharedRes->peers[c]);
  ...
  INFO(NCCL_INIT,
       "comm %p rank %d nranks %d cudaDev %d busId %lx - %s COMPLETE", comm,
...
static ncclResult_t dmaBufSupported(struct ncclComm *comm) {
  if (comm->ncclNet->regMrDmaBuf == NULL || rocmLibraryInit() != ncclSuccess)
  ...
  return ncclSuccess;
#else
  return pfn_hsa_amd_portable_export_dmabuf != NULL ? ncclSuccess
                                                    : ncclInternalError;
#endif
  return ncclInternalError;
}
...
ncclResult_t ncclCommEnsureReady(ncclComm_t comm) {
  ...
  if (__atomic_load_n(comm->abortFlag, __ATOMIC_ACQUIRE)) {
    ...
    if (ret != ncclSuccess) {
      ...
      WARN("Attempt to use communicator before the previous operation returned "
           "ncclSuccess");
...
static ncclResult_t commAlloc(struct ncclComm *comm, struct ncclComm *parent,
                              int ndev, int rank) {
  if (ndev < 1) {
  ...
  if (parent && parent->config.splitShare) {
    if (parent->ncclNet != comm->ncclNet) {
      WARN("Split shares resources, but parent comm netName %s is different "
           "from child comm netName %s",
  ...
  NCCLCHECK(
      rocm_smi_getDeviceIndexByPciBusId(busId, (unsigned int *)&comm->nvmlDev));

  comm->compCap = ncclCudaCompCap();
  TRACE(NCCL_INIT, "comm %p rank %d nranks %d cudaDev %d busId %lx compCap %d",
        comm, rank, ndev, comm->cudaDev, comm->busId, comm->compCap);
  ...
  NCCLCHECK(
      ncclCudaCalloc(&comm->collTrace, COLLTRACE_NUM_ITEMS * MAXCHANNELS));
  comm->collTraceExit = 0;
  comm->collTraceEnabled =
      false; // we can enable colltrace without starting a thread
  if ((ncclDebugLevel >= NCCL_LOG_INFO) && rcclParamKernelCollTraceEnable()) {
    ...
    if (rcclParamKernelCollTraceThreadEnable())
      pthread_create(&comm->collTraceThread, NULL, ncclCommThreadMain,
                     (void *)comm);
    else
  ...
  if (rcclParamInjectFaults() != 0) {
    ...
    if (comm->rank == 0)
      INFO(NCCL_INIT, "Enabled RCCL faults injection with value 0x%lx",
           comm->faults);
#else
    WARN("Ignore faults injection of value 0x%lx as RCCL is not compiled to "
         "support it",
         rcclParamInjectFaults());
#endif
  }
  ...
  CUDACHECK(hipDeviceGetAttribute(&comm->WarpSize, hipDeviceAttributeWarpSize,
                                  comm->cudaDev));
  ...
  do {
    ...
    CUDACHECK(cudaMemPoolSetAttribute(
        comm->memPool, cudaMemPoolAttrReleaseThreshold, &releaseThreshold));
  } while (0);
...
static ncclResult_t devCommSetup(ncclComm_t comm) {
  ...
  NCCLCHECKGOTO(
      ncclStrongStreamAcquireUncaptured(&comm->sharedRes->deviceStream), ret,
      fail);
  NCCLCHECKGOTO(ncclCudaCallocAsync(&devCommAndChans, 1,
                                    comm->sharedRes->deviceStream.cudaStream),
                ret, fail);
  ncclCommPushCudaFree(comm, devCommAndChans);
  NCCLCHECKGOTO(ncclCudaCallocAsync(&tmpCommAndChans.comm.rankToLocalRank,
                                    comm->nRanks,
                                    comm->sharedRes->deviceStream.cudaStream),
                ret, fail);
  ncclCommPushCudaFree(comm, tmpCommAndChans.comm.rankToLocalRank);
  NCCLCHECKGOTO(ncclCudaMemcpyAsync(tmpCommAndChans.comm.rankToLocalRank,
                                    comm->rankToLocalRank, comm->nRanks,
                                    comm->sharedRes->deviceStream.cudaStream),
                ret, fail);
  ...
  comm->workArgsBytes = std::min<size_t>(ncclParamWorkArgsBytes(),
                                         ncclMaxKernelArgsSize(comm->cudaArch));

#if !defined(__HIP_PLATFORM_AMD__) && !defined(__HIPCC__)
  memset(&ccStatus, 0, sizeof(ccStatus));
  ccEnable = (ncclSuccess == ncclNvmlGetCCStatus(&ccStatus)) &&
             (ccStatus.CCEnabled || ccStatus.multiGpuProtectedPCIE);
  if (ccEnable) {
    comm->workFifoBytes = 0;
  } else {
    comm->workFifoBytes = ncclParamWorkFifoBytes();
    if (0 != (comm->workFifoBytes & (comm->workFifoBytes - 1))) {
      WARN("NCCL_WORK_FIFO_BYTES=%d is being ignored because it is not a power "
           "of 2.",
           comm->workFifoBytes);
  ...
  if (comm->rank == 0) {
    INFO(NCCL_INIT, "CC %s, workFifoBytes %d", ccEnable ? "On" : "Off",
         comm->workFifoBytes);
  }

  if (ncclGdrCopy != NULL && ncclParamGdrCopyFifoEnable() == 1) {
    // The workFifoBuf lives in GDR mapped CUDA memory.
    NCCLCHECKGOTO(ncclGdrCudaCalloc(&comm->workFifoBuf, &comm->workFifoBufDev,
                                    comm->workFifoBytes,
    ...
    NCCLCHECKGOTO(ncclCudaHostCalloc(&comm->workFifoBuf, comm->workFifoBytes),
                  ret, fail);
    ncclCommPushCudaHostFree(comm, comm->workFifoBuf);
    comm->workFifoBufDev = comm->workFifoBuf;
  }

  NCCLCHECKGOTO(ncclCudaHostCalloc(&comm->workFifoConsumed, MAXCHANNELS), ret,
                fail);
  ...
  if (comm->collNetDenseToUserRank != nullptr) {
    NCCLCHECKGOTO(
        ncclCudaCallocAsync(&tmpCommAndChans.comm.collNetDenseToUserRank,
                            nRanks, comm->sharedRes->deviceStream.cudaStream),
        ret, fail);
    ncclCommPushCudaFree(comm, tmpCommAndChans.comm.collNetDenseToUserRank);
    NCCLCHECKGOTO(
        ncclCudaMemcpyAsync(tmpCommAndChans.comm.collNetDenseToUserRank,
                            comm->collNetDenseToUserRank, nRanks,
                            comm->sharedRes->deviceStream.cudaStream),
        ret, fail);
  }
  ...
  for (int c = 0; c < MAXCHANNELS; c++) {
    ...
    if (comm->channels[c].ring.userRanks != nullptr) {
      NCCLCHECKGOTO(
          ncclCudaMemcpyAsync(tmpCommAndChans.channels[c].ring.userRanks,
                              comm->channels[c].ring.userRanks, nRanks,
                              comm->sharedRes->deviceStream.cudaStream),
          ret, fail);
    }
  }
  ...
  tmpCommAndChans.comm.npKitEventCollectContexts =
      NpKit::GetGpuEventCollectContexts();
  tmpCommAndChans.comm.cpuTimestamp = NpKit::GetCpuTimestamp();
#endif

#ifdef ENABLE_PROFILING
  NCCLCHECK(ncclCudaCalloc(&tmpCommAndChans.comm.devProf,
                           MAXCHANNELS * PROFILE_NUM_LAUNCHES,
                           comm->sideStream));
#endif
  ...
  NCCLCHECKGOTO(ncclCudaMemcpyAsync(devCommAndChans, &tmpCommAndChans, 1,
                                    comm->sharedRes->deviceStream.cudaStream),
                ret, fail);
exit:
  NCCLCHECK(ncclStrongStreamSynchronize(&comm->sharedRes->deviceStream));
  NCCLCHECK(ncclStrongStreamRelease(ncclCudaGraphNone(),
...
// Pre-process the string so that running "strings" on the lib can quickly
// reveal the version.
#if defined(__HIP_PLATFORM_AMD__) || defined(__HIPCC__)
#define VERSION_STRING                                                         \
  "RCCL version : " STR(NCCL_MAJOR) "." STR(NCCL_MINOR) "." STR(NCCL_PATCH)    \
      NCCL_SUFFIX
#define VERSION_STRING_EXTENDED                                                \
  "HIP version  : " HIP_BUILD_INFO "\nROCm version : " ROCM_BUILD_INFO
#else
#define VERSION_STRING                                                         \
  "NCCL version " STR(NCCL_MAJOR) "." STR(NCCL_MINOR) "." STR(NCCL_PATCH)      \
      NCCL_SUFFIX
#define VERSION_STRING_EXTENDED                                                \
  "CUDA version " STR(CUDA_MAJOR) "." STR(CUDA_MINOR)
#endif
static void showVersion() {
  char versionInfo[2048 + 2 * HOST_NAME_MAX], hostInfo[HOST_NAME_MAX],
      libPathInfo[2048];
  ...
  snprintf(versionInfo, sizeof(versionInfo),
           "%s-%s\n%s\n"
           "%-12s : %s\n%12s : %s",
           VERSION_STRING, rcclGitHash, VERSION_STRING_EXTENDED, "Hostname",
           hostInfo, "Librccl path", libPathInfo);
...
static ncclResult_t fillInfo(struct ncclComm *comm, struct ncclPeerInfo *info,
  ...
  // MNNVL support
  {
    ...
    if (info->fabricInfo.state != NVML_GPU_FABRIC_STATE_NOT_SUPPORTED) {
      ...
      if (ncclParamMNNVLCliqueId() != -1)
        info->fabricInfo.cliqueId = ncclParamMNNVLCliqueId();
      INFO(NCCL_INIT,
           "MNNVL busId 0x%lx fabric UUID %lx.%lx cliqueId 0x%x state %d "
           "healthMask 0x%x",
           info->busId, ((long *)&info->fabricInfo.clusterUuid)[0],
           ((long *)&info->fabricInfo.clusterUuid)[1],
           info->fabricInfo.cliqueId, info->fabricInfo.state,
...
static ncclResult_t setupChannel(struct ncclComm *comm, int channelId, int rank,
                                 int nranks, int *ringRanks) {
  TRACE(NCCL_INIT, "rank %d nranks %d", rank, nranks);
  NCCLCHECK(initChannel(comm, channelId));

  struct ncclRing *ring = &comm->channels[channelId].ring;
  // Find our ring-distance from rank zero and reorganize ranks to start with
  // rank.
...
static ncclResult_t computeBuffSizes(struct ncclComm *comm) {
  ...
  if (comm->nNodes > 1)
    comm->p2pChunkSize = ncclParamP2pNetChunkSize();
...
static ncclResult_t initTransportsRank(struct ncclComm *comm,
  ...
  struct ncclTopoGraph *graphs[NCCL_NUM_ALGORITHMS] = {
      treeGraph, ringGraph, collNetDirectGraph, collNetChainGraph,
  ...
  // AllGather1 - begin
  NCCLCHECKGOTO(ncclCalloc(&comm->peerInfo, nranks + 1), ret,
                fail); // Extra rank to represent CollNet root
  NCCLCHECKGOTO(fillInfo(comm, comm->peerInfo + rank, comm->commHash), ret,
                fail);
  NCCLCHECKGOTO(bootstrapAllGather(comm->bootstrap, comm->peerInfo,
  ...
  for (int i = 0; i < nranks; i++) {
    if (comm->peerInfo[i].version != comm->peerInfo[rank].version) {
      WARN("Mismatched NCCL version detected : rank %d version %d rank %d "
           "version %d",
  ...
  if ((nNodes > 1 && ncclParamMNNVLEnable() != 0) ||
      ncclParamMNNVLEnable() == 1) {
  ...
  do {
    ...
    for (int i = 0; i < nranks; i++)
      comm->minCompCap =
          std::min(comm->minCompCap, comm->peerInfo[i].cudaCompCap);
    for (int i = 0; i < nranks; i++)
      comm->maxCompCap =
          std::max(comm->maxCompCap, comm->peerInfo[i].cudaCompCap);
    ...
    TRACE(
        NCCL_INIT,
        "pidHash[%d] %lx intraProcRank %d intraProcRanks %d intraProcRank0 %d",
        rank, comm->peerInfo[rank].pidHash, intraProcRank, intraProcRanks,
        intraProcRank0);
    if (intraProcRank == -1 || intraProcRank0 == -1 ||
        comm->peerInfo[intraProcRank0].comm == NULL) {
      WARN("Failed to determine intra proc ranks rank %d hostHash %lx pidHash "
           "%lx intraProcRank %d intraProcRanks %d intraProcRank0 %d",
  ...
  NCCLCHECKGOTO(
      ncclTopoGetCpuAffinity(comm->topo, comm->rank, &comm->cpuAffinity), ret,
      fail);
  ...
  if (collNetSupport(comm)) {
    ...
    if (collNetEnable != NULL) {
      INFO(NCCL_ALL, "NCCL_COLLNET_ENABLE set by environment to %s.",
           collNetEnable);
  ...
  if (comm->collNetSupport) {
    ...
    NCCLCHECKGOTO(ncclTopoPrintGraph(comm->topo, collNetDirectGraph), ret,
                  fail);
  }
  ...
  allXgmi = true;
  hasPeerAccess = true;
  // Check that all the GPUs have peer access to one another and are XGMI
  // connected
  ...
  if (comm->rank == ncclParamGraphDumpFileRank()) {
    struct ncclTopoGraph *dumpGraphs[5] = {
        ringGraph, treeGraph, collNetDirectGraph, collNetChainGraph, nvlsGraph};
    NCCLCHECKGOTO(ncclTopoDumpGraphs(comm->topo, 5, dumpGraphs), ret, fail);
  }

  if ((comm->topo->type & RCCL_TOPO_4P2H_ROME) &&
      (comm->topo->type & RCCL_TOPO_GDR_ALL)) {
    if (rcclParamP2pNetDisable() == 0) {
      if (!(comm->topo->type & RCCL_TOPO_FORCE_INTRA))
        comm->p2pNet = 1;
      INFO(NCCL_INIT, "RCCL enabled same node P2P over network");
    } else
      INFO(NCCL_INIT, "RCCL force disabled same node P2P over network");
  }
  // Because timers[[TIMER_INIT_ALLGATHER] already contains the timing of the
  // first allgather, we temporarily store the start time of the subsequent one
  // in an as-of-yet unused CONNECT timer.
  ...
  if (comm->topo->nodes[GPU].count == comm->topo->nRanks &&
      IsArchMatch(comm->topo->nodes[GPU].nodes[idx].gpu.gcn, "gfx906") &&
      allXgmi)
  ...
  if (IsArchMatch(comm->topo->nodes[GPU].nodes[idx].gpu.gcn, "gfx90a"))
    allGather3Data[rank].nc =
        std::max(allGather3Data[rank].nc, 4 / ringGraph->nChannels);
  if (ringGraph->nChannels > MAXCHANNELS / 2)
    allGather3Data[rank].nc = 1;
  if (IsArchMatch(comm->topo->nodes[GPU].nodes[idx].gpu.gcn, "gfx942")) {
    // Multi-node MI300A
    int managed = 0;
    CUDACHECK(hipDeviceGetAttribute(
        &managed, hipDeviceAttributeDirectManagedMemAccessFromHost, 0));
  ...
  allGather3Data[rank].pivotA2AEnabled =
      comm->topo->pivotA2AEnabled && rcclParamPivotAlltoallEnable();
  comm->topo->ll128Enabled =
      comm->topo->ll128Enabled || rcclParamLL128ForceEnable();
  ...
  NCCLCHECKGOTO(ncclTopoPreset(comm, graphs, &allGather3Data[rank].topoRanks),
                ret, fail);

  NCCLCHECKGOTO(bootstrapAllGather(comm->bootstrap, allGather3Data,
                                   sizeof(*allGather3Data)),
                ret, fail);
  ...
  for (int r = 0; r < nranks; r++) {
    ...
    for (node = 0; node < comm->nNodes && nodesFirstRank[node] != firstRank;
         node++)
      ;
    if (node == comm->nNodes) {
      comm->nNodes++;
      nodesFirstRank[node] = firstRank;
      // Record tree pattern of each node as they can be different depending on
      // sm arch
      nodesTreePatterns[node] =
          allGather3Data[r].graphInfo[NCCL_ALGO_TREE].pattern;
    }
  ...
  // locks in some collective routines. This may help debug issues in the
  // future.
  if (rank == 0) {
  ...
  // Now that we know nNodes, alloc nodeRanks and compute localRanks for each
  // node
  ...
  for (int n = 0; n < comm->nNodes; n++) {
    NCCLCHECKGOTO(ncclCalloc(&comm->nodeRanks[n].localRankToRank,
  ...
  for (int r = 0; r < comm->nRanks; r++) {
    int node = comm->rankToNode[r];
    comm->nodeRanks[node].localRankToRank[comm->nodeRanks[node].localRanks++] =
        r;
  }
  ...
  if (comm->localRank == -1 || comm->localRankToRank[0] == -1 ||
      comm->localRanks == 0) {
    WARN("Failed to determine local ranks rank %d hostHash %lx pidHash %lx "
         "localRank %d localRanks %d localRank0 %d",
         rank, comm->peerInfo[rank].hostHash, comm->peerInfo[rank].pidHash,
  ...
  INFO(
      NCCL_INIT,
      "comm %p rank %d nRanks %d nNodes %d localRanks %d localRank %d MNNVL %d",
      comm, rank, comm->nRanks, comm->nNodes, comm->localRanks, comm->localRank,
      comm->MNNVL);
  ...
  for (int i = 0; i < nranks; i++) {
    ...
    // Make sure we align all ranks so that the tuning is consistent across
    // ranks
    comm->topo->pivotA2AEnabled =
        comm->topo->pivotA2AEnabled && allGather3Data[i].pivotA2AEnabled;
    comm->topo->ll128Enabled =
        comm->topo->ll128Enabled && allGather3Data[i].ll128Enabled;
    comm->topo->mscclEnabled =
        comm->topo->mscclEnabled && allGather3Data[i].mscclEnabled;
    for (int a = 0; a < NCCL_NUM_ALGORITHMS; a++) {
      graphs[a]->nChannels = std::min(allGather3Data[i].graphInfo[a].nChannels,
                                      graphs[a]->nChannels);
      graphs[a]->sameChannels = std::min(
          allGather3Data[i].graphInfo[a].sameChannels, graphs[a]->sameChannels);
      graphs[a]->bwIntra =
          std::min(allGather3Data[i].graphInfo[a].bwIntra, graphs[a]->bwIntra);
      graphs[a]->bwInter =
          std::min(allGather3Data[i].graphInfo[a].bwInter, graphs[a]->bwInter);
      graphs[a]->typeIntra = std::max(allGather3Data[i].graphInfo[a].typeIntra,
                                      graphs[a]->typeIntra);
      graphs[a]->typeInter = std::max(allGather3Data[i].graphInfo[a].typeInter,
                                      graphs[a]->typeInter);
      graphs[a]->crossNic = std::max(allGather3Data[i].graphInfo[a].crossNic,
                                     graphs[a]->crossNic);
    }
  }
  ...
  if (comm->nChannels < nChannelsOrig) {
    ...
    for (int i = 0; i < comm->nChannels; i++)
      memcpy(comm->channels + comm->nChannels + i,
             comm->channels + nChannelsOrig + i, sizeof(struct ncclChannel));
  }

  // Determine CollNet support after all-gather now that we know nNodes and each
  // node localRanks
  if (comm->collNetSupport == 1) {
    int collNetNodeThreshold = ncclParamCollNetNodeThreshold();
    if (comm->nNodes < collNetNodeThreshold) {
      INFO(NCCL_INIT,
           "Communicator has %d nodes which is less than CollNet node "
           "threshold %d, disabling CollNet",
  ...
  NCCLCHECKGOTO(ncclTopoPostset(comm, nodesFirstRank, nodesTreePatterns,
                                allTopoRanks, rings, graphs, parent, nc),
                ret, fail);
  if (comm->topo->treeDefined)
    NCCLCHECK(ncclTreeBasePostset(comm, treeGraph));

  // AllGather3 - end
  timers[TIMER_INIT_ALLGATHER] += clockNano() - timers[TIMER_INIT_CONNECT];

  TRACE(NCCL_INIT, "rank %d nranks %d - BUILT %d TREES/RINGS", rank, nranks,
        comm->nChannels);
  ...
  for (int c = 0; c < comm->nChannels; c++) {
    ...
    INFO(NCCL_GRAPH, "Ring %d : %d -> %d -> %d comm %p nRanks %02d busId %lx",
         c, comm->channels[c].ring.prev, comm->rank,
         comm->channels[c].ring.next, comm, comm->nRanks, comm->busId);
  }
  line[4095] = '\0';
  INFO(NCCL_INIT, "Trees%s comm %p nRanks %02d busId %lx", line, comm,
       comm->nRanks, comm->busId);
  ...
  /* until now, all info of comm should be known. We can initialize shared
   * resources and map localRanks to top parent local ranks. NOTE: this shareRes
   * init must be put before all proxy operations. */
  if (comm->sharedRes->owner == comm) {
  ...
  NCCLCHECKGOTO(ncclTransportCheckP2pType(comm, &comm->intraNodeP2pSupport,
                                          &comm->directMode),
                ret, fail);
  ...
  do { // Build p2p schedule
    ...
    comm->p2pSchedule = ncclMemoryStackAlloc<ncclComm::P2pSchedulePair>(
        &comm->memPermanent, nRanks);
    comm->planner.peers = ncclMemoryStackAlloc<ncclKernelPlanner::Peer>(
        &comm->memPermanent, nRanks);
    uint32_t nodeRound = 0;
    uint32_t nodeDelta = 0;
    int round = 0;
    // When enumerating peer deltas we use the quadratic formula (x*x+x)/2 mod
    // N. Since that formula only produces valid permutations when N is a pow of
    // 2, we let N = pow2Up(n) and filter out results greater-eq to n. Example
    // sequence for 16 ranks: 0, 1, 3, 6, 10, 15, 5, 12, 4, 13, 7, 2, 14, 11, 9,
    // 8
    do {
      ...
      nodeRound += 1;
      nodeDelta =
          (nodeDelta + nodeRound) & (nNodesPow2 - 1); // Quadratic update
  ...
  if (comm->runtimeConn) {
    for (int c = 0; c < comm->nChannels; c++) {
      NCCLCHECKGOTO(setupChannel(comm, c, rank, nranks, rings + c * nranks),
                    ret, fail);
    }
    // Setup NVLS
    ...
    if (comm->graphs[NCCL_ALGO_RING].nIntraChannels &&
        rcclParamP2pNetDisable() == 0) {
      comm->useIntraNet = 1;
      for (int c = 0; c < comm->nChannels; c++) {
        struct ncclChannel *channel = comm->channels + c;
        if (comm->nRanks == 1)
          continue;
        NCCLCHECKGOTO(ncclTransportP2pConnect(comm, c, 1, &channel->ring.prev,
                                              1, &channel->ring.next,
                                              NCCL_CONN_IDX_P2P_NET),
                      ret, fail);
      }
      NCCLCHECKGOTO(ncclTransportP2pSetup(comm, &comm->graphs[NCCL_ALGO_RING],
                                          NCCL_CONN_IDX_P2P_NET),
                    ret, fail);
    }
    ...
    NCCLCHECKGOTO(
        ncclProxyConnect(comm, TRANSPORT_NET, 1, comm->rank, &proxyConn), ret,
        fail);
    NCCLCHECKGOTO(
        ncclProxyCallBlocking(comm, &proxyConn, ncclProxyMsgSharedInit,
                              &comm->p2pnChannels, sizeof(int), NULL, 0),
        ret, fail);
    ...
    if (ncclPxnDisable(comm) == 0) {
      ...
      for (int r = 0; r < nranks; r++) {
        NCCLCHECKGOTO(
            ncclProxyConnect(comm, TRANSPORT_NET, 1, pxnPeers[r], &proxyConn),
            ret, fail);
        NCCLCHECKGOTO(
            ncclProxyCallBlocking(comm, &proxyConn, ncclProxyMsgSharedInit,
                                  &comm->p2pnChannels, sizeof(int), NULL, 0),
    ...
    if (ncclParamNvbPreconnect()) {
      ...
      int nvbNpeers;
      NCCLCHECKGOTO(
          ncclTopoGetNvbGpus(comm->topo, comm->rank, &nvbNpeers, &nvbPeers),
          ret, fail);
      for (int r = 0; r < nvbNpeers; r++) {
        ...
        for (int c = 0; c < comm->p2pnChannelsPerPeer; c++) {
          int channelId;
          channelId =
              ncclP2pChannelForPart(comm->p2pnChannels, sendBase, c,
                                    comm->p2pnChannelsPerPeer, comm->nNodes);
          if (comm->channels[channelId].peers[peer]->send[1].connected == 0) {
            comm->connectSend[peer].masks[channelId / 64] |=
                (1UL << (channelId % 64));
          }
          channelId =
              ncclP2pChannelForPart(comm->p2pnChannels, recvBase, c,
                                    comm->p2pnChannelsPerPeer, comm->nNodes);
          if (comm->channels[channelId].peers[peer]->recv[1].connected == 0) {
            comm->connectRecv[peer].masks[channelId / 64] |=
                (1UL << (channelId % 64));
          }
        }
      }
  ...
  TRACE(NCCL_INIT, "rank %d nranks %d - CONNECTED %d RINGS AND TREES", rank,
        nranks, comm->nChannels);

  // Compute time models for algorithm and protocol combinations
  NCCLCHECKGOTO(
      ncclTopoTuneModel(comm, comm->minCompCap, comm->maxCompCap, graphs), ret,
      fail);

  INFO(NCCL_INIT,
       "%d coll channels, %d collnet channels, %d nvls channels, %d p2p "
       "channels, %d p2p channels per peer",
       comm->nChannels, comm->nChannels, comm->nvlsChannels, comm->p2pnChannels,
  ...
  if (comm->intraRank == 0) { // Load ncclParamLaunchMode
    ...
    // this one if the user is connecting to multiple ncclUniqueId's
    // concurrently.
  ...
  // Call devCommSetup before the last barrier, making sure we don't have a
  // thread running in front and starting to launch NCCL kernels before all cuda
  // mem allocation is complete. That could cause a deadlock.
  ...
  NCCLCHECKGOTO(bootstrapIntraNodeBarrier(
...
exit:
  ...
  /* If split resource is shared, we are not able to unlink the proxy ops pool
   * here since the child comm can attach the proxy ops pool of parent at any
   * time; otherwise, unlink it here to make sure the pool will be properly
   * cleaned up. */
  if (comm->sharedRes->owner == comm && !comm->config.splitShare &&
      ret == ncclSuccess && !ncclCuMemEnable())
...
NCCL_PARAM(CommSplitShareResources, "COMM_SPLIT_SHARE_RESOURCES",
...
static ncclResult_t commGetSplitInfo(struct ncclComm *comm,
                                     struct ncclComm *parent, int color,
                                     int key, int *nRanksRet, int *myRankRet,
                                     int *parentRanksRet) {
  ...
  // Compute nRanks, my rank and the ranks (of the original comm) before and
  // after me
  info[parent->rank].color = color;
  info[parent->rank].key = key;
  NCCLCHECKGOTO(
      bootstrapAllGather(parent->bootstrap, info, sizeof(commSplitInfo)), ret,
      fail);
  ...
  for (int i = 0; i < parent->nRanks; i++) {
    ...
    int insert = 0;
    while (insert < nRanks && info[parentRanksRet[insert]].key <= info[i].key)
      insert++;
    // Shift ranks by one after insert
    for (int r = nRanks; r > insert; r--)
      parentRanksRet[r] = parentRanksRet[r - 1];
    // Insert our rank
  ...
  for (int i = 0; i < nRanks; i++) {
    if (parentRanksRet[i] == parent->rank)
      myRank = i;
  }
...
static ncclResult_t ncclCommInitRankFunc(struct ncclAsyncJob *job_) {
  struct ncclCommInitRankAsyncJob *job =
      (struct ncclCommInitRankAsyncJob *)job_;
  ...
  if (ncclParamSetStackSize() == 1 &&
      !IsArchMatch(devProp.gcnArchName, "gfx942") &&
      !IsArchMatch(devProp.gcnArchName, "gfx950")) {
    stackSize = rcclParamStackSizeOverride() ? rcclParamStackSizeOverride()
                                             : maxLocalSizeBytes;
    if (stackSize == 0) {
    ...
    INFO(NCCL_INIT, "Setting cudaLimitStackSize to %zi maxLocalSizeBytes %zi",
         stackSize, maxLocalSizeBytes);
  ...
  if (job->parent) {
    ...
    NCCLCHECKGOTO(commGetSplitInfo(comm, job->parent, job->color, job->key,
                                   &job->nranks, &job->myrank, parentRanks),
                  res, fail);
    // Negative color does not create a new comm object. We needed to take part
    // in the allgather, but we're done now.
    if (job->color == NCCL_SPLIT_NOCOLOR)
      goto exit;
    timers[TIMER_INIT_ALLOC] = clockNano();
    NCCLCHECKGOTO(commAlloc(comm, job->parent, job->nranks, job->myrank), res,
                  fail);
    timers[TIMER_INIT_ALLOC] = clockNano() - timers[TIMER_INIT_ALLOC];
    // obtain a unique hash for the comm, re-using part of the parent's hash,
    // commHash is a 64bit struct (=16 hex), add unique split counter and the
    // color
    ncclUniqueId tmpId;
    memset(&tmpId, 0,
           sizeof(ncclUniqueId)); // must set 0 here to avoid undefined bits
    snprintf((char *)&tmpId, NCCL_UNIQUE_ID_BYTES, "%016lx-%d-%d",
             job->parent->commHash, job->splitCount, job->color);
    comm->commHash = getHash(tmpId.internal, NCCL_UNIQUE_ID_BYTES);
    INFO(NCCL_INIT,
         "%s comm %p rank %d nranks %d cudaDev %d nvmlDev %d busId %lx parent "
         "%p splitCount %d color %d key %d- Init START",
         job->funcName, comm, comm->rank, comm->nRanks, comm->cudaDev,
         comm->nvmlDev, comm->busId, job->parent, job->splitCount, job->color,
         job->key);
    timers[TIMER_INIT_BOOTSTRAP] = clockNano();
    NCCLCHECKGOTO(bootstrapSplit(comm->commHash, comm, job->parent, job->color,
                                 job->key, parentRanks),
                  res, fail);
    ...
    INFO(NCCL_INIT,
         ...
         "0x%llx - Init START",
         job->funcName, comm, comm->rank, comm->nRanks, comm->cudaDev,
         comm->nvmlDev, comm->busId, commIdHash);
    timers[TIMER_INIT_BOOTSTRAP] = clockNano();
    NCCLCHECKGOTO(bootstrapInit(job->nId,
                                (struct ncclBootstrapHandle *)job->commId,
                                comm),
                  res, fail);
    timers[TIMER_INIT_BOOTSTRAP] = clockNano() - timers[TIMER_INIT_BOOTSTRAP];
  }
  ...
  if (job->parent) {
    if (job->parent->mscclppCompatible) {
      INFO(NCCL_INIT, "MSCCL++: Splitting a compatible communicator; using "
                      "parent mscclpp_comm");
...
#ifdef ENABLE_MSCCLPP
    if (mscclEnabled() && (comm->topo->mscclEnabled || mscclForceEnabled()) &&
        mscclppCommCompatible(comm)) {
      hipDeviceProp_t devProp;
      CUDACHECK(hipGetDeviceProperties(&devProp, cudaDev));
      comm->mscclppCompatible = IsArchMatch(devProp.gcnArchName, "gfx942") ||
                                IsArchMatch(devProp.gcnArchName, "gfx950");
      if (comm->mscclppCompatible) {
        ...
        if (comm->localRank == 0 && !mapContainsId) {
          ...
          TRACE_CALL("mscclpp_ncclGetUniqueId(0x%llx)",
                     (unsigned long long)hashUniqueId(mscclppUniqueId));
        }

        NCCLCHECKGOTO(bootstrapIntraNodeBroadcast(
                          comm->bootstrap, comm->localRankToRank,
                          comm->localRank, comm->localRanks, 0,
                          &mscclppUniqueId, sizeof(mscclppUniqueId)),
                      res, fail);
        unsigned long long mscclppUniqueIdHash;
        (void)mscclppUniqueIdHash;
        TRACE_CALL("bootstrapIntraNodeBroadcast(rank=%d, nranks=%d, root=%d, "
                   "bcastData=hash:0x%llx)",
                   comm->localRank, comm->localRanks, 0,
                   (mscclppUniqueIdHash =
                        (unsigned long long)hashUniqueId(mscclppUniqueId)));
        mscclpp_uniqueIdReverseMap[mscclppUniqueId].insert(*job->commId);
        ...
        INFO(NCCL_INIT, "MSCCL++: Enabled! Msg size threshold=%zu",
             comm->mscclpp_threshold);

        NCCLCHECKGOTO(mscclpp_ncclCommInitRank(&(comm->mscclpp_comm),
                                               job->nranks, mscclppUniqueId,
                                               job->myrank),
                      res, fail);
        TRACE_CALL("mscclpp_ncclCommInitRank (*comm=%p, nranks=%d, "
                   "commId=hash:0x%llx, myrank=%d)",
                   comm->mscclpp_comm, job->nranks, mscclppUniqueIdHash,
                   job->myrank);
        ...
        WARN("MSCCL++: Cannot enable MSCCL++ on %s architecture",
             devProp.gcnArchName);
      }
    } else {
      comm->mscclppCompatible = false;
      WARN("MSCCL++: Cannot enable MSCCL++; environment is not MSCCL "
           "compatible");
    }
#else
    WARN("MSCCL++: Feature not enabled. ENABLE_MSCCLPP must be defined at "
  ...
  if (job->parent) {
    ...
    TRACE_CALL("ncclCommSplit(%p, %d, %d, %p, %d, %d)", job->parent, job->color,
               job->key, comm, comm->rank, comm->nRanks);
    INFO(NCCL_INIT,
         "%s comm %p rank %d nranks %d cudaDev %d nvmlDev %d busId %lx parent "
         "%p splitCount %d color %d key %d - Init COMPLETE",
         job->funcName, comm, comm->rank, comm->nRanks, comm->cudaDev,
         comm->nvmlDev, comm->busId, job->parent, job->splitCount, job->color,
         job->key);
  } else {
    // the name for the replay tool is ncclCommInitRank for all the variations
    TRACE_CALL("ncclCommInitRank(%p, %d, 0x%llx, %d, %d)", comm, comm->nRanks,
               commIdHash, comm->rank, comm->cudaDev);
    INFO(NCCL_INIT,
         "%s comm %p rank %d nranks %d cudaDev %d nvmlDev %d busId %lx commId "
         "0x%llx - Init COMPLETE",
         job->funcName, comm, comm->rank, comm->nRanks, comm->cudaDev,
         comm->nvmlDev, comm->busId, commIdHash);
  }
  sum_timers = 0.0;
  ...
  INFO(NCCL_INIT | NCCL_PROFILE,
       ...
       job->funcName, comm->rank, comm->nRanks, timers[TIMER_INIT_TOTAL] / 1e9,
       timers[TIMER_INIT_KERNELS] / 1e9, timers[TIMER_INIT_ALLOC] / 1e9,
       timers[TIMER_INIT_BOOTSTRAP] / 1e9, timers[TIMER_INIT_ALLGATHER] / 1e9,
       timers[TIMER_INIT_TOPO] / 1e9, timers[TIMER_INIT_GRAPHS] / 1e9,
       timers[TIMER_INIT_CONNECT] / 1e9,
...
static ncclResult_t envConfigOverride(ncclComm_t comm) {
  ...
  if (0 <= cgaClusterSizeEnv &&
      cgaClusterSizeEnv <= NCCL_MAX_CGA_CLUSTER_SIZE) {
    comm->config.cgaClusterSize = cgaClusterSizeEnv;
  } else if (cgaClusterSizeEnv > NCCL_MAX_CGA_CLUSTER_SIZE) {
    INFO(NCCL_ENV,
         "NCCL_CGA_CLUSTER_SIZE value %d is too big. Limiting value to %d.",
         cgaClusterSizeEnv, NCCL_MAX_CGA_CLUSTER_SIZE);
  ...
  if (minCTAsEnv != NCCL_CONFIG_UNDEF_INT) {
    if (minCTAsEnv <= 0)
      INFO(NCCL_ENV, "NCCL_MIN_CTAS %d is too low, leaving it set at %d",
           minCTAsEnv, comm->config.minCTAs);
    else
  ...
  if (maxCTAsEnv != NCCL_CONFIG_UNDEF_INT) {
    if (maxCTAsEnv <= 0)
      INFO(NCCL_ENV, "NCCL_MAX_CTAS %d is too low, leaving it set at %d",
           maxCTAsEnv, comm->config.maxCTAs);
    else
  ...
  if (comm->config.minCTAs > MAXCHANNELS) {
    INFO(NCCL_ENV,
         "minCTAs %d is larger than #channels upper limit %d, cap it to %d",
         comm->config.minCTAs, MAXCHANNELS, MAXCHANNELS);
    comm->config.minCTAs = MAXCHANNELS;
  }

  if (comm->config.maxCTAs > MAXCHANNELS) {
    INFO(NCCL_ENV,
         "maxCTAs %d is larger than #channels upper limit %d, cap it to %d",
         comm->config.maxCTAs, MAXCHANNELS, MAXCHANNELS);
    comm->config.maxCTAs = MAXCHANNELS;
  }

  if (comm->config.minCTAs > comm->config.maxCTAs) {
    INFO(NCCL_ENV, "minCTAs %d is larger than maxCTAs %d, set both to %d",
         comm->config.minCTAs, comm->config.maxCTAs, comm->config.maxCTAs);
    comm->config.minCTAs = comm->config.maxCTAs;
  }

  if (comm->config.splitShare != 1 && comm->config.splitShare != 0) {
    INFO(NCCL_ENV, "splitShare %d is not a valid value 0/1, set it to 0",
...
static ncclResult_t parseCommConfig(ncclComm_t comm, ncclConfig_t *config) {
  ...
  if (config) {
    ...
    realSize =
        realSize > sizeof(ncclConfig_t) ? sizeof(ncclConfig_t) : realSize;
  ...
  /* check input config attributes, -1 means user-undefined and we should use
   * default value from NCCL. */
  if (internalConfigPtr->blocking != NCCL_CONFIG_UNDEF_INT &&
      internalConfigPtr->blocking != 0 && internalConfigPtr->blocking != 1) {
    WARN("Invalid config blocking attribute value %d",
         internalConfigPtr->blocking);
    ret = ncclInvalidArgument;
    goto fail;
  }

  if (internalConfigPtr->cgaClusterSize != NCCL_CONFIG_UNDEF_INT &&
      internalConfigPtr->cgaClusterSize < 0) {
    WARN("Invalid config cgaClusterSize attribute value %d",
         internalConfigPtr->cgaClusterSize);
    ret = ncclInvalidArgument;
    goto fail;
  }
  ...
  if ((internalConfigPtr->minCTAs != NCCL_CONFIG_UNDEF_INT &&
    ...
    WARN("Invalid config min/max channels attribute value %d/%d",
         internalConfigPtr->minCTAs, internalConfigPtr->maxCTAs);
    ret = ncclInvalidArgument;
    goto fail;
  }

  if (internalConfigPtr->splitShare != NCCL_CONFIG_UNDEF_INT &&
      internalConfigPtr->splitShare != 0 &&
      internalConfigPtr->splitShare != 1) {
    WARN("Invalid config splitShare attribute value %d",
         internalConfigPtr->splitShare);
    ret = ncclInvalidArgument;
    goto fail;
  }

  /* default config value can be tuned on different platform. */
  NCCL_CONFIG_DEFAULT(internalConfigPtr, blocking, NCCL_CONFIG_UNDEF_INT, 1,
                      "Blocking", "%d");
  NCCL_CONFIG_DEFAULT(internalConfigPtr, cgaClusterSize, NCCL_CONFIG_UNDEF_INT,
                      4, "CGA cluster size", "%d");
  NCCL_CONFIG_DEFAULT(internalConfigPtr, minCTAs, NCCL_CONFIG_UNDEF_INT, 1,
                      "Min CTAs", "%d");
  NCCL_CONFIG_DEFAULT(internalConfigPtr, maxCTAs, NCCL_CONFIG_UNDEF_INT,
                      MAXCHANNELS, "Max CTAs", "%d");
  NCCL_CONFIG_DEFAULT(internalConfigPtr, netName, NCCL_CONFIG_UNDEF_PTR, NULL,
                      "Net name", "%s");
  NCCL_CONFIG_DEFAULT(internalConfigPtr, splitShare, NCCL_CONFIG_UNDEF_INT, 0,
                      "Split share", "%d");
...
static void ncclCommInitJobFree(void *_job) {
  struct ncclCommInitRankAsyncJob *job =
      (struct ncclCommInitRankAsyncJob *)_job;
  free(job->commId);
  free(_job);
}

static ncclResult_t ncclCommInitRankDev(ncclComm_t *newcomm, int nranks,
                                        int nId, ncclUniqueId *commId,
                                        int myrank, int cudaDev,
                                        ncclConfig_t *config,
                                        const char funcName[]) {
  if (nId <= 0 || nId > nranks) {
    WARN("improper usage of ncclCommInitRank: nId = %d, nranks=%d", nId,
         nranks);
    return ncclInvalidArgument;
  }
  ncclResult_t res = ncclSuccess;
  ...
  if (ncclDebugLevel > NCCL_LOG_WARN ||
      (ncclDebugLevel != NCCL_LOG_NONE && myrank == 0)) {
  ...
  *comm->abortFlagRefCount = 1;
  ...
  /* start with ncclInProgress and will be changed to ncclSuccess if init
   * succeeds. */
  ...
  *newcomm = comm;
  ...
  // need to copy the commIds to allow async commInit and to avoid alignement
  // issues when casting from ncclUNiqueId and ncclBootstrapHandle ncclUniqueIds
  // and ncclBootstrapHandle don't have the same alignment requirements.
  // Therefore the array of Ids coming from the user might not be properly
  // aligned to be cast into a ncclBootstrapHandle copying into allocated memory
  // guarantees that the memory is properly aligned for any objects, removing
  // that issue
  ...
  if (commIdEnv && myrank == 0) {
    ...
    if (nId > 1) {
      INFO(NCCL_INIT | NCCL_ENV,
           "NCCL_COMM_ID cannot be used with more than one ncclUniqueId");
      job->nId = 1;
    }
    // start the bootstrap root before bootstrapping, use only the first handle
    NCCLCHECKGOTO(bootstrapCreateRoot(
                      (struct ncclBootstrapHandle *)&job->commId[0], true),
                  res, fail);
  }
  NCCLCHECKGOTO(ncclAsyncLaunch((struct ncclAsyncJob *)job,
                                ncclCommInitRankFunc, NULL, ncclCommInitJobFree,
                                comm),
                res, fail);
...
exit:
  ...
  // !recording at sink
  NCCLCHECK(Recorder::instance().record(rrCommInitDev, nranks, myrank, commId,
                                        comm, cudaDev));
...
ncclResult_t ncclCommInitRank_impl(ncclComm_t *newcomm, int nranks,
                                   ncclUniqueId commId, int myrank) {
  NCCLCHECK(
      Recorder::instance().record(rrCommInitRank, nranks, myrank, &commId));
  ...
  NCCLCHECK(ncclCommInitRankDev(newcomm, nranks, 1, &commId, myrank, cudaDev,
                                &config, __func__));

  NVTX3_RANGE_ADD_PAYLOAD(
      CommInitRank, NcclNvtxParamsCommInitRankSchema,
      NVTX3_PAYLOAD((*newcomm)->commHash, nranks, myrank, cudaDev));

  return ncclSuccess;
}

NCCL_API(ncclResult_t, ncclCommInitAll, ncclComm_t *comms, int ndev,
         const int *devlist);
ncclResult_t ncclCommInitAll_impl(ncclComm_t *comms, int ndev,
                                  const int *devlist) {
...
ncclResult_t ncclCommInitRankConfig_impl(ncclComm_t *newcomm, int nranks,
                                         ncclUniqueId commId, int myrank,
                                         ncclConfig_t *config) {
  Recorder::instance().record(rrCommInitRankConfig, nranks, myrank, &commId,
                              config);
  int cudaDev;
  ncclResult_t ret = ncclSuccess;
  ncclConfig_t internalConfig = NCCL_CONFIG_INITIALIZER;
  ...
  NCCLCHECKGOTO(ncclCommInitRankDev(newcomm, nranks, 1, &commId, myrank,
                                    cudaDev, internalConfigPtr, __func__),
                ret, fail);
...
exit:
  ...
  if (newcomm && *newcomm) {
    ...
    NVTX3_RANGE_ADD_PAYLOAD(
        CommInitRankConfig, NcclNvtxParamsCommInitRankSchema,
        NVTX3_PAYLOAD((*newcomm)->commHash, nranks, myrank, cudaDev));
  }
  return ret;
fail:
  if (newcomm && *newcomm && !(*newcomm)->config.blocking)
    (void)ncclCommSetAsyncError(*newcomm, ret);
  goto exit;
}

NCCL_API(ncclResult_t, ncclCommInitRankScalable, ncclComm_t *newcomm,
         int nranks, int myrank, int nId, ncclUniqueId *commId,
         ncclConfig_t *config);
ncclResult_t ncclCommInitRankScalable(ncclComm_t *newcomm, int nranks,
                                      int myrank, int nId, ncclUniqueId *commId,
                                      ncclConfig_t *config) {
  ...
  if (config == NULL)
    ...
    internalConfigPtr = config;
  NCCLCHECKGOTO(ncclCommInitRankDev(newcomm, nranks, nId, commId, myrank,
                                    cudaDev, internalConfigPtr, __func__),
                ret, fail);
...
exit:
  ...
  if (newcomm && *newcomm) {
    ...
    NVTX3_RANGE_ADD_PAYLOAD(
        CommInitRankScalable, NcclNvtxParamsCommInitRankSchema,
        NVTX3_PAYLOAD((*newcomm)->commHash, nranks, myrank, cudaDev));
  }
  return ret;
fail:
...
static ncclResult_t commDestroySync(struct ncclAsyncJob *job_) {
  struct ncclCommFinalizeAsyncJob *job =
      (struct ncclCommFinalizeAsyncJob *)job_;
  ncclComm_t comm = job->comm;
  ncclResult_t ret = ncclSuccess;

  CUDACHECKGOTO(cudaSetDevice(comm->cudaDev), ret, fail);

  TRACE(NCCL_INIT, "Destroying comm %p rank %d abortFlag %d asyncResult %d",
        comm, comm->rank, *comm->abortFlag, comm->asyncResult);

  if (comm->initState == ncclSuccess) {
    if ((ret = ncclStrongStreamSynchronize(&comm->sharedRes->hostStream)) !=
        ncclSuccess) {
      WARN("commDestroySync: comm %p rank %d sync hostStream error %d\n", comm,
           comm->rank, ret);
    }
    if ((ret = ncclStrongStreamSynchronize(&comm->sharedRes->deviceStream)) !=
        ncclSuccess) {
    ...
    while (!ncclIntruQueueEmpty(&comm->legacyRegCleanupQueue)) {
      struct ncclCommCallback *cb =
          ncclIntruQueueDequeue(&comm->legacyRegCleanupQueue);
      if (cb->fn(comm, cb) != ncclSuccess) {
        WARN("Legacy IPC cleanup callback failed comm %p (rank = %d) cb %p",
             comm, comm->rank, cb);
      }
    }
  }
...
ncclResult_t ncclCommFinalize_impl(ncclComm_t comm) {
  ...
  job->comm = comm;
  NCCLCHECKGOTO(ncclAsyncLaunch((struct ncclAsyncJob *)job, commDestroySync,
                                NULL, free, comm),
                ret, fail);
...
static ncclResult_t commReclaim(struct ncclAsyncJob *job_) {
  ...
  if (comm->intraComm0 != NULL) {
    ...
    if (curRankCnt == intraRanks) {
      ...
      /* this is  the last call to ncclCommDestroy/Abort, we need to make sure
       * all comms in the process have been finalized before we free local
       * resources. */
      while (nextIntraComm) {
        ...
        if (curIntraComm->finalizeCalled == false) {
          ...
          if ((ret = commDestroySync((struct ncclAsyncJob *)&job)) !=
              ncclSuccess)
            WARN(
                "commReclaim: comm %p (rank = %d) in commDestroySync, error %d",
                curIntraComm, curRank, ret);
        }
      }
      ...
      while (nextIntraComm) {
        ...
        if ((ret = commCleanup(curIntraComm)) != ncclSuccess) {
          // We pass a freed pointer, but we don't dereference; we merely print
          // its value, so it's OK. coverity[pass_freed_arg]
          WARN("commReclaim: cleanup comm %p rank %d failed in destroy/abort, "
               "error %d",
               curIntraComm, curRank, ret);
        }
      }
...
ncclResult_t ncclCommDestroy_impl(ncclComm_t comm) {
  ...
  if (comm->mscclppCompatible) {
    ...
    if (uniqueIds.find(ncclUniqueId) == uniqueIds.end()) {
      WARN(
          "MSCCL++: comm=%p not found in mscclpp_uniqueIdReverseMap for key=%p",
          comm, comm->mscclpp_comm);
    }
    ...
    if (uniqueIds.size() == 0) {
      ...
      if (res != ncclSuccess) {
        WARN("MSCCL++: mscclpp_ncclCommDestroy failed (%s)",
  ...
  TRACE(NCCL_INIT, "comm %p rank %d nRanks %d cudaDev %d busId %lx", comm, rank,
        nranks, cudaDev, comm->busId);
  NCCLCHECK(ncclGroupStartInternal());
  // Try and prevent a double free of the comm struct (user error)
  if (comm->rank == -1 || comm->nRanks == -1 || comm->cudaDev == -1 ||
      comm->busId == -1) {
  ...
  NCCLCHECKGOTO(ncclCalloc(&job, 1), res, fail);
  job->comm = comm;
  NCCLCHECKGOTO(ncclAsyncLaunch((struct ncclAsyncJob *)job, commReclaim, NULL,
                                free, comm),
                res, fail);
...
ncclResult_t ncclCommSplit_impl(ncclComm_t comm, int color, int key,
                                ncclComm_t *newcomm, ncclConfig_t *config) {
  struct ncclCommInitRankAsyncJob *job = NULL;
  ...
  *newcomm = NCCL_COMM_NULL;
  if (color == NCCL_SPLIT_NOCOLOR) {
    INFO(NCCL_INIT,
         "Rank %d has color with NCCL_SPLIT_NOCOLOR, not creating a new "
         "communicator",
         comm->rank);
  } else {
  ...
  NCCLCHECKGOTO(ncclAsyncLaunch((struct ncclAsyncJob *)job,
                                ncclCommInitRankFunc, NULL, free, comm),
                res, fail);
...
exit:
  ...
  // !recording at sink
  Recorder::instance().record(rrCommSplit, color, key, (ncclUniqueId *)comm,
                              config, *newcomm);
  (void)cudaSetDevice(oldDev);
  ...
  if (res == ncclSuccess && *newcomm) {
    NVTX3_RANGE_ADD_PAYLOAD(CommSplit, NcclNvtxParamsCommSplitSchema,
                            NVTX3_PAYLOAD((*newcomm)->commHash, comm->commHash,
                                          comm->nRanks, comm->rank,
                                          comm->cudaDev, color, key));
  }
...
NCCL_API(ncclResult_t, ncclCommGetAsyncError, ncclComm_t comm,
         ncclResult_t *asyncError);
ncclResult_t ncclCommGetAsyncError_impl(ncclComm_t comm,
                                        ncclResult_t *asyncError) {
...
ncclResult_t ncclMemAlloc_impl(void **ptr, size_t size) {
  ...
  if (ncclCuMemEnable()) {
    ...
    if (CUPFN(cuMulticastCreate) != NULL)
      CUCHECK(cuDeviceGetAttribute(
          &mcSupport, CU_DEVICE_ATTRIBUTE_MULTICAST_SUPPORTED, currentDev));
    if (mcSupport) {
      /* mc property */
      mcprop.size = size;
      /* device cnt is a dummy value right now, it might affect mc granularity
       * in the future. */
      mcprop.numDevices = dcnt;
      mcprop.handleTypes = requestedHandleTypes;
      mcprop.flags = 0;
      CUCHECK(cuMulticastGetGranularity(&mcGran, &mcprop,
    ...
    if (requestedHandleTypes & CU_MEM_HANDLE_TYPE_FABRIC) {
      /* First try cuMemCreate() with FABRIC handle support and then remove if
       * it fails */
    ...
    for (int i = 0; i < dcnt; ++i) {
      int p2p = 0;
      if (i == cudaDev ||
          ((cudaDeviceCanAccessPeer(&p2p, cudaDev, i) == cudaSuccess) && p2p)) {
      ...
      if (0 == p2p && i != cudaDev)
        INFO(NCCL_ALLOC, "P2P not supported between GPU%d and GPU%d", cudaDev,
             i);
    }
    goto exit;
  }
...
fallback:
#endif
  // Coverity is right to complain that we may pass a NULL ptr to cudaMalloc.
  // That's deliberate though: we want CUDA to return an error to the caller.
...
ncclResult_t ncclMemFree_impl(void *ptr) {
  ...
  CUCHECKGOTO(cuPointerGetAttribute((void *)&ptrDev,
