...
__global__ void RcclKernelScalarReduce(RingNode_t* pcurr_track, const void* send_buff, void* recv_buff, unsigned count) {
    ...
    if(pnext_track != pcurr_track) {
        ...
        if(Op == rcclMax) {
            dest_buff[tid] = curr_buff[tid] > next_buff[tid] ? curr_buff[tid] : next_buff[tid];
        }
        if(Op == rcclMin) {
            dest_buff[tid] = curr_buff[tid] < next_buff[tid] ? curr_buff[tid] : next_buff[tid];
        }
    ...
    while(pnext_track != pcurr_track) {
        ...
        if(Op == rcclMax) {
            dest_buff[tid] = dest_buff[tid] > next_buff[tid] ? dest_buff[tid] : next_buff[tid];
        }
        if(Op == rcclMin) {
            dest_buff[tid] = dest_buff[tid] < next_buff[tid] ? dest_buff[tid] : next_buff[tid];
        }
...
// Disabled operating on last elements on a different kernel
#if 0
...
__global__ void RcclKernelReduce(RingNode_t* pcurr_track, const void* send_buff, void* recv_buff, unsigned offset, unsigned num_scalars) {
    ...
    if(pnext_track != pcurr_track) {
        ...
        // get source buffer from peer gpu
        DataType_t* next_buff = reinterpret_cast<DataType_t*>(std::atomic_load_explicit(&(pnext_track->src_buffer), std::memory_order_seq_cst)) + offset;
