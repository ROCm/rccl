...
namespace RcclUnitTesting
{
  ...
  public:
    ...
    bool                       verbose;               // Output extra messages to stdout (set by env var UT_VERBOSE)
    ...
    // Using collId = -1 (default) applies settings to all collectives in group
    // Using rank = -1 (default) applies settings to all ranks
    void AllocateMem(size_t const numInputBytes,
    ...
    void ValidateResults(int collId = -1, int const rank = -1);
    ...
    static std::vector<ncclRedOp_t> GetAllSupportedRedOps();

    // Return all the supported data types based on build settings
    static std::vector<ncclDataType_t> GetAllSupportedDataTypes();
