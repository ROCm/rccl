...
* Experimental support for traffic shaping using warp specialization (also known as WarpSpeed) is now available for the Ring algorithm. The following configuration knobs control its behavior: `RCCL_WARP_SPEED_ENABLE`, `RCCL_UNROLL_FACTOR`, `RCCL_WARP_SPEED_CU_COUNT`, and `RCCL_THREADS_PER_BLOCK`. Note that the effective unroll factor is calculated as 2 raised to the value of `RCCL_UNROLL_FACTOR`.
