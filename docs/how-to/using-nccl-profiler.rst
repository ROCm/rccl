...
Profiler generated errors should not be propagated to NCCL and alter its normal
functioning. The profiler interface returns NCCL error codes, in case any need for
them arises. Any profiler interface call should only return ``ncclSuccess``.
...
returns an opaque profiler event handle that can be passed to other profiler functions.

The event descriptor contains all the event metadata. Every event type has its own descriptor. 

Here is the ``ncclProfilerEventDescr_t`` struct:
...
``ncclProfileP2p``, can't be updated through calls to ``recordEventState``.
...
Here's the list of the supported states for the updatable events:
