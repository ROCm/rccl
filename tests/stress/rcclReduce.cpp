...
void CallReduce(signed char* psrc_buff, signed char* pdst_buff, size_t buff_len, rcclRedOp_t op, int root, rcclComm_t comm, hipStream_t stream) {
    RCCLCHECK(rcclReduce(psrc_buff, pdst_buff, buff_len, rcclChar, op, root, comm, stream));
}

void CallReduce(unsigned char* psrc_buff, unsigned char* pdst_buff, size_t buff_len, rcclRedOp_t op, int root, rcclComm_t comm, hipStream_t stream) {
    RCCLCHECK(rcclReduce(psrc_buff, pdst_buff, buff_len, rcclUchar, op, root, comm, stream));
}

void CallReduce(signed short* psrc_buff, signed short* pdst_buff, size_t buff_len, rcclRedOp_t op, int root, rcclComm_t comm, hipStream_t stream) {
    RCCLCHECK(rcclReduce(psrc_buff, pdst_buff, buff_len, rcclShort, op, root, comm, stream));
}

void CallReduce(unsigned short* psrc_buff, unsigned short* pdst_buff, size_t buff_len, rcclRedOp_t op, int root, rcclComm_t comm, hipStream_t stream) {
    RCCLCHECK(rcclReduce(psrc_buff, pdst_buff, buff_len, rcclUshort, op, root, comm, stream));
}

void CallReduce(signed int* psrc_buff, signed int* pdst_buff, size_t buff_len, rcclRedOp_t op, int root, rcclComm_t comm, hipStream_t stream) {
    RCCLCHECK(rcclReduce(psrc_buff, pdst_buff, buff_len, rcclInt, op, root, comm, stream));
}

void CallReduce(unsigned int* psrc_buff, unsigned int* pdst_buff, size_t buff_len, rcclRedOp_t op, int root, rcclComm_t comm, hipStream_t stream) {
    RCCLCHECK(rcclReduce(psrc_buff, pdst_buff, buff_len, rcclUint, op, root, comm, stream));
}

void CallReduce(signed long* psrc_buff, signed long* pdst_buff, size_t buff_len, rcclRedOp_t op, int root, rcclComm_t comm, hipStream_t stream) {
    RCCLCHECK(rcclReduce(psrc_buff, pdst_buff, buff_len, rcclLong, op, root, comm, stream));
}

void CallReduce(unsigned long* psrc_buff, unsigned long* pdst_buff, size_t buff_len, rcclRedOp_t op, int root, rcclComm_t comm, hipStream_t stream) {
    RCCLCHECK(rcclReduce(psrc_buff, pdst_buff, buff_len, rcclUlong, op, root, comm, stream));
}

void CallReduce(__fp16* psrc_buff, __fp16* pdst_buff, size_t buff_len, rcclRedOp_t op, int root, rcclComm_t comm, hipStream_t stream) {
    RCCLCHECK(rcclReduce(psrc_buff, pdst_buff, buff_len, rcclHalf, op, root, comm, stream));
}

void CallReduce(float* psrc_buff, float* pdst_buff, size_t buff_len, rcclRedOp_t op, int root, rcclComm_t comm, hipStream_t stream) {
    RCCLCHECK(rcclReduce(psrc_buff, pdst_buff, buff_len, rcclFloat, op, root, comm, stream));
}

void CallReduce(double* psrc_buff, double* pdst_buff, size_t buff_len, rcclRedOp_t op, int root, rcclComm_t comm, hipStream_t stream) {
    RCCLCHECK(rcclReduce(psrc_buff, pdst_buff, buff_len, rcclDouble, op, root, comm, stream));
}
...
void DoReduce(std::vector<int>& device_list, std::vector<hipStream_t>& device_streams,
    ...
    for(int i = 0; i < device_list.size(); i++) {
        for(size_t j = 0; j < buff_len; j++) {
            reinterpret_cast<T*>(host_buffers[i])[j] = static_cast<T>(kbuffer_values[device_list[i]]);
        }
...
// free allocted buffers on both host and device
    ...
    for(auto iter = device_buffers.begin(); iter != device_buffers.end(); iter++) {
