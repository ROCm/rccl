...
void RandomGatherTest(std::vector<int>& device_list, int num_tests) {
    ...
    { // used new scope to force current-device guard to destruct after changing active device
     ...
     for(auto pbuff_len = buffer_lengths.begin(); pbuff_len != buffer_lengths.end(); pbuff_len++) {
        ...
        DoAllGather<__fp16>(device_list, device_streams, rccl_comms, src_host_buffers, src_device_buffers, dst_host_buffers, dst_device_buffers, *pbuff_len);
    }
...
int main(int argc, char* argv[]) {
    ...
    if(argc != 3 || argc != 6) {
        ...
        std::cout<<"[-r] enabled validation for certain number of gpus, elements, and datatypes"<<std::endl;
        return 0;
    }
}
