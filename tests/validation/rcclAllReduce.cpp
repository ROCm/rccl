        ...
        for (size_t i = 0; i < num_gpus; i++) {
            ...
            if (IsInPlace == true) {
                HIPCHECK(hipMemcpyAsync(
                    dst_device_buffers[i], src_host_buffers[i], buff_size,
                    hipMemcpyHostToDevice, device_streams[i]));
                CallAllReduce(reinterpret_cast<T*>(dst_device_buffers[i]),
                              reinterpret_cast<T*>(dst_device_buffers[i]),
                              buff_len, p_ops->second, rccl_comms[i],
                              device_streams[i]);
            } else {
...
void RandomReduceTest(std::vector<int>& device_list, int num_tests) {
    ...
    for (auto pbuff_len = buffer_lengths.begin();
        ...
        DoAllReduce<signed char, true>(device_list, device_streams, rccl_comms,
                                       src_host_buffers, src_device_buffers,
                                       dst_host_buffers, dst_device_buffers,
                                       *pbuff_len);
        DoAllReduce<unsigned char, true>(device_list, device_streams,
                                         rccl_comms, src_host_buffers,
                                         src_device_buffers, dst_host_buffers,
                                         dst_device_buffers, *pbuff_len);
        DoAllReduce<signed short, true>(device_list, device_streams, rccl_comms,
                                        src_host_buffers, src_device_buffers,
                                        dst_host_buffers, dst_device_buffers,
                                        *pbuff_len);
        DoAllReduce<unsigned short, true>(device_list, device_streams,
                                          rccl_comms, src_host_buffers,
                                          src_device_buffers, dst_host_buffers,
                                          dst_device_buffers, *pbuff_len);
        DoAllReduce<signed int, true>(device_list, device_streams, rccl_comms,
                                      src_host_buffers, src_device_buffers,
                                      dst_host_buffers, dst_device_buffers,
                                      *pbuff_len);
        DoAllReduce<unsigned int, true>(device_list, device_streams, rccl_comms,
                                        src_host_buffers, src_device_buffers,
                                        dst_host_buffers, dst_device_buffers,
                                        *pbuff_len);
        DoAllReduce<signed long, true>(device_list, device_streams, rccl_comms,
                                       src_host_buffers, src_device_buffers,
                                       dst_host_buffers, dst_device_buffers,
                                       *pbuff_len);
        DoAllReduce<unsigned long, true>(device_list, device_streams,
                                         rccl_comms, src_host_buffers,
                                         src_device_buffers, dst_host_buffers,
                                         dst_device_buffers, *pbuff_len);
        DoAllReduce<float, true>(device_list, device_streams, rccl_comms,
                                 src_host_buffers, src_device_buffers,
                                 dst_host_buffers, dst_device_buffers,
                                 *pbuff_len);
        DoAllReduce<double, true>(device_list, device_streams, rccl_comms,
                                  src_host_buffers, src_device_buffers,
                                  dst_host_buffers, dst_device_buffers,
                                  *pbuff_len);
        DoAllReduce<__fp16, true>(device_list, device_streams, rccl_comms,
                                  src_host_buffers, src_device_buffers,
                                  dst_host_buffers, dst_device_buffers,
                                  *pbuff_len);
    }
    ...
    for (auto pbuff_len = buffer_lengths.begin();
         pbuff_len != buffer_lengths.end(); pbuff_len++) {
        DoAllReduce<signed char, false>(device_list, device_streams, rccl_comms,
                                        src_host_buffers, src_device_buffers,
                                        dst_host_buffers, dst_device_buffers,
                                        *pbuff_len);
        DoAllReduce<unsigned char, false>(device_list, device_streams,
                                          rccl_comms, src_host_buffers,
                                          src_device_buffers, dst_host_buffers,
                                          dst_device_buffers, *pbuff_len);
        DoAllReduce<signed short, false>(device_list, device_streams,
                                         rccl_comms, src_host_buffers,
                                         src_device_buffers, dst_host_buffers,
                                         dst_device_buffers, *pbuff_len);
        DoAllReduce<unsigned short, false>(device_list, device_streams,
                                           rccl_comms, src_host_buffers,
                                           src_device_buffers, dst_host_buffers,
                                           dst_device_buffers, *pbuff_len);
        DoAllReduce<signed int, false>(device_list, device_streams, rccl_comms,
                                       src_host_buffers, src_device_buffers,
                                       dst_host_buffers, dst_device_buffers,
                                       *pbuff_len);
        DoAllReduce<unsigned int, false>(device_list, device_streams,
                                         rccl_comms, src_host_buffers,
                                         src_device_buffers, dst_host_buffers,
                                         dst_device_buffers, *pbuff_len);
        DoAllReduce<signed long, false>(device_list, device_streams, rccl_comms,
                                        src_host_buffers, src_device_buffers,
                                        dst_host_buffers, dst_device_buffers,
                                        *pbuff_len);
        DoAllReduce<unsigned long, false>(device_list, device_streams,
                                          rccl_comms, src_host_buffers,
                                          src_device_buffers, dst_host_buffers,
                                          dst_device_buffers, *pbuff_len);
        DoAllReduce<float, false>(device_list, device_streams, rccl_comms,
                                  src_host_buffers, src_device_buffers,
                                  dst_host_buffers, dst_device_buffers,
                                  *pbuff_len);
        DoAllReduce<double, false>(device_list, device_streams, rccl_comms,
                                   src_host_buffers, src_device_buffers,
                                   dst_host_buffers, dst_device_buffers,
                                   *pbuff_len);
        DoAllReduce<__fp16, true>(device_list, device_streams, rccl_comms,
...
void ReduceTestSize(std::vector<int>& device_list, size_t size_in_bytes) {
    ...
    DoAllReduce<signed char, false>(device_list, device_streams, rccl_comms,
                                    ...
                                    dst_host_buffers, dst_device_buffers,
                                    size_in_bytes);
    DoAllReduce<unsigned char, false>(device_list, device_streams, rccl_comms,
                                      src_host_buffers, src_device_buffers,
                                      dst_host_buffers, dst_device_buffers,
                                      size_in_bytes);
    DoAllReduce<signed short, false>(device_list, device_streams, rccl_comms,
                                     src_host_buffers, src_device_buffers,
                                     dst_host_buffers, dst_device_buffers,
                                     size_in_bytes);
    DoAllReduce<unsigned short, false>(device_list, device_streams, rccl_comms,
                                       src_host_buffers, src_device_buffers,
                                       dst_host_buffers, dst_device_buffers,
                                       size_in_bytes);
    DoAllReduce<signed int, false>(device_list, device_streams, rccl_comms,
                                   ...
                                   dst_host_buffers, dst_device_buffers,
                                   size_in_bytes);
    DoAllReduce<unsigned int, false>(device_list, device_streams, rccl_comms,
                                     src_host_buffers, src_device_buffers,
                                     dst_host_buffers, dst_device_buffers,
                                     size_in_bytes);
    DoAllReduce<signed long, false>(device_list, device_streams, rccl_comms,
                                    ...
                                    dst_host_buffers, dst_device_buffers,
                                    size_in_bytes);
    DoAllReduce<unsigned long, false>(device_list, device_streams, rccl_comms,
                                      src_host_buffers, src_device_buffers,
                                      dst_host_buffers, dst_device_buffers,
                                      size_in_bytes);
    DoAllReduce<float, false>(device_list, device_streams, rccl_comms,
    ...
    DoAllReduce<double, false>(device_list, device_streams, rccl_comms,
                               ...
                               dst_host_buffers, dst_device_buffers,
                               size_in_bytes);
    DoAllReduce<__fp16, false>(device_list, device_streams, rccl_comms,
                               src_host_buffers, src_device_buffers,
                               dst_host_buffers, dst_device_buffers,
                               size_in_bytes);
...
int main(int argc, char* argv[]) {
    ...
    if (argc != 3 || argc != 5) {
        ...
        std::cout << "Usage: ./a.out <num gpus> <size of buffer in bytes>" << std::endl;
