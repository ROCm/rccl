...
template<typename T>
void DoAllReduce(std::vector<int>& device_list, std::vector<hipStream_t>& htod_streams, std::vector<hipStream_t>& op_streams, std::vector<hipStream_t>& dtoh_streams,
    ...
    for(int i = 0; i < device_list.size(); i++) {
        ...
        HIPCHECK(hipMemcpyAsync(src_device_buffers[i], src_host_buffers[i], buff_size, hipMemcpyHostToDevice, htod_streams[i]));
    ...
    for(auto p_ops = umap_rccl_op.begin(); p_ops != umap_rccl_op.end(); p_ops++) {
        for(size_t i = 0; i < num_gpus; i++) {
            ...
            CallAllReduce(reinterpret_cast<T*>(src_device_buffers[i]), reinterpret_cast<T*>(dst_device_buffers[i]), buff_len, p_ops->second, rccl_comms[i], op_streams[i]);
        ...
        for(size_t i = 0; i < num_gpus; i++) {
            ...
            HIPCHECK(hipMemcpyAsync(dst_host_buffers[i], dst_device_buffers[i], buff_size, hipMemcpyDeviceToHost, dtoh_streams[i]));
...
//        dst_host_buffers[i] = reinterpret_cast<void*>(new signed char[max_allocated_memory]);
    ...
    { // used new scope to force current-device guard to destruct after changing active device
     ...
     for(auto pbuff_len = buffer_lengths.begin(); pbuff_len != buffer_lengths.end(); pbuff_len++) {
        DoAllReduce<signed char>(device_list, htod_streams, op_streams, dtoh_streams, rccl_comms, src_host_buffers, src_device_buffers, dst_host_buffers, dst_device_buffers, *pbuff_len);
        DoAllReduce<unsigned char>(device_list, htod_streams, op_streams, dtoh_streams, rccl_comms, src_host_buffers, src_device_buffers, dst_host_buffers, dst_device_buffers, *pbuff_len);
        DoAllReduce<signed short>(device_list, htod_streams, op_streams, dtoh_streams, rccl_comms, src_host_buffers, src_device_buffers, dst_host_buffers, dst_device_buffers, *pbuff_len);
        DoAllReduce<unsigned short>(device_list, htod_streams, op_streams, dtoh_streams, rccl_comms, src_host_buffers, src_device_buffers, dst_host_buffers, dst_device_buffers, *pbuff_len);
        DoAllReduce<signed int>(device_list, htod_streams, op_streams, dtoh_streams, rccl_comms, src_host_buffers, src_device_buffers, dst_host_buffers, dst_device_buffers, *pbuff_len);
        DoAllReduce<unsigned int>(device_list, htod_streams, op_streams, dtoh_streams, rccl_comms, src_host_buffers, src_device_buffers, dst_host_buffers, dst_device_buffers, *pbuff_len);
        DoAllReduce<signed long>(device_list, htod_streams, op_streams, dtoh_streams, rccl_comms, src_host_buffers, src_device_buffers, dst_host_buffers, dst_device_buffers, *pbuff_len);
        DoAllReduce<unsigned long>(device_list, htod_streams, op_streams, dtoh_streams,  rccl_comms, src_host_buffers, src_device_buffers, dst_host_buffers, dst_device_buffers, *pbuff_len);
        DoAllReduce<float>(device_list, htod_streams, op_streams, dtoh_streams, rccl_comms, src_host_buffers, src_device_buffers, dst_host_buffers, dst_device_buffers, *pbuff_len);
        DoAllReduce<double>(device_list, htod_streams, op_streams, dtoh_streams, rccl_comms, src_host_buffers, src_device_buffers, dst_host_buffers, dst_device_buffers, *pbuff_len);
...
void ReduceTestSize(std::vector<int>& device_list, size_t size_in_bytes) {
    ...
    { // used new scope to force current-device guard to destruct after changing active device
        ...
        DoAllReduce<signed char>(device_list, htod_streams, op_streams, dtoh_streams, rccl_comms, src_host_buffers, src_device_buffers, dst_host_buffers, dst_device_buffers, size_in_bytes);
        DoAllReduce<unsigned char>(device_list, htod_streams, op_streams, dtoh_streams, rccl_comms, src_host_buffers, src_device_buffers, dst_host_buffers, dst_device_buffers, size_in_bytes);
        DoAllReduce<signed short>(device_list, htod_streams, op_streams, dtoh_streams, rccl_comms, src_host_buffers, src_device_buffers, dst_host_buffers, dst_device_buffers, size_in_bytes);
        DoAllReduce<unsigned short>(device_list, htod_streams, op_streams, dtoh_streams, rccl_comms, src_host_buffers, src_device_buffers, dst_host_buffers, dst_device_buffers, size_in_bytes);
        DoAllReduce<signed int>(device_list, htod_streams, op_streams, dtoh_streams, rccl_comms, src_host_buffers, src_device_buffers, dst_host_buffers, dst_device_buffers, size_in_bytes);
        DoAllReduce<unsigned int>(device_list, htod_streams, op_streams, dtoh_streams, rccl_comms, src_host_buffers, src_device_buffers, dst_host_buffers, dst_device_buffers, size_in_bytes);
        DoAllReduce<signed long>(device_list, htod_streams, op_streams, dtoh_streams, rccl_comms, src_host_buffers, src_device_buffers, dst_host_buffers, dst_device_buffers, size_in_bytes);
        DoAllReduce<unsigned long>(device_list, htod_streams, op_streams, dtoh_streams, rccl_comms, src_host_buffers, src_device_buffers, dst_host_buffers, dst_device_buffers, size_in_bytes);
        DoAllReduce<float>(device_list, htod_streams, op_streams, dtoh_streams, rccl_comms, src_host_buffers, src_device_buffers, dst_host_buffers, dst_device_buffers, size_in_bytes);
        DoAllReduce<double>(device_list, htod_streams, op_streams, dtoh_streams, rccl_comms, src_host_buffers, src_device_buffers, dst_host_buffers, dst_device_buffers, size_in_bytes);
