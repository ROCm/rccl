...
void CallBcast(float* psrc_buff, size_t buff_len, int root, rcclComm_t comm,
               hipStream_t stream) {
    RCCLCHECK(rcclBcast(psrc_buff, buff_len, rcclFloat, root, comm, stream));
}

void CallBcast(double* psrc_buff, size_t buff_len, int root, rcclComm_t comm,
               hipStream_t stream) {
    RCCLCHECK(rcclBcast(psrc_buff, buff_len, rcclDouble, root, comm, stream));
}

template <typename T>
void DoBcast(std::vector<int>& device_list,
             ...
             std::vector<void*>& dst_host_buffers, size_t buff_size, int root) {
...
void RandomReduceTest(std::vector<int>& device_list, int num_tests, int root) {
    ...
    for (auto pbuff_len = buffer_lengths.begin();
        ...
        DoBcast<signed long>(device_list, device_streams, rccl_comms,
                             src_device_buffers, src_host_buffers,
                             dst_host_buffers, *pbuff_len, root);
        DoBcast<unsigned long>(device_list, device_streams, rccl_comms,
                               src_device_buffers, src_host_buffers,
                               dst_host_buffers, *pbuff_len, root);
        DoBcast<float>(device_list, device_streams, rccl_comms,
                       src_device_buffers, src_host_buffers, dst_host_buffers,
                       *pbuff_len, root);
        DoBcast<double>(device_list, device_streams, rccl_comms,
                        src_device_buffers, src_host_buffers, dst_host_buffers,
                        *pbuff_len, root);
        DoBcast<__fp16>(device_list, device_streams, rccl_comms,
                        src_device_buffers, src_host_buffers, dst_host_buffers,
                        *pbuff_len, root);
    }
