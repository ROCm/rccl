    ...
    for (auto p_ops = umap_rccl_op.begin(); p_ops != umap_rccl_op.end();
        ...
        for (size_t i = 0; i < num_gpus; i++) {
            ...
            CallReduce(reinterpret_cast<T*>(device_buffers[i]),
                       ...
                       p_ops->second, root, rccl_comms[i], device_streams[i]);
        }
        for (size_t i = 0; i < num_gpus; i++) {
            HIPCHECK(hipSetDevice(device_list[i]));
        ...
        HIPCHECK(hipMemcpy(dst_host_buffer, dst_device_buffer, buff_size,
        ...
        if (p_ops->second == rcclSum) {
            ...
            validate(reinterpret_cast<T*>(dst_host_buffer), sum_val, buff_len,
                     1, 0);
        }
        if (p_ops->second == rcclProd) {
            ...
            validate(reinterpret_cast<T*>(dst_host_buffer), prod_val, buff_len,
                     1, 0);
        }
        if (p_ops->second == rcclMax) {
            ...
            validate(reinterpret_cast<T*>(dst_host_buffer), max_val, buff_len,
                     1, 0);
        }
        if (p_ops->second == rcclMin) {
            ...
            validate(reinterpret_cast<T*>(dst_host_buffer), min_val, buff_len,
                     1, 0);
        }
    }
}
